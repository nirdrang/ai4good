# Design log â€” Lovable implementation track

> Running log per the design-session playbook (B5 + per-batch entries). Newest last.

## 2026-07-22 â€” Bring-up B5: Lovable baseline
...
- **âš  d42 drift flag (2026-07-22):** the missing credit field is a reproducible drift from a
  validated product assumption â€” decision d42 recorded "pull Lovable credit status via
  ...
  return neither credits nor member count. The product's Lovable-chip design leans on d42 â€”
  ...
  **Doc research 2026-07-22:** docs.lovable.dev still documents the field AND states credit
  information surfaces ONLY through `get_workspace` (no billing endpoint exists) â€” so today
  ...
  exists for some Claude client surfaces (not exposed to this Claude Code connection) â€” watch
  item: could simplify the Â§2 handoff if it becomes available.
- Gate status at baseline: design gate 001 = CONDITIONAL FAIL (change order 002 open with
  Claude Design â€” requirement vocabulary + status authority). **No build message goes to
...
## 2026-07-22 â€” Batch 0 gate PASS + bring-up B1 done
...
- **B1 project-knowledge push DONE (free, no credits):** replaced the AI4GOOD project's Lovable
  knowledge with the enriched rule set â€” preserved lane/boundary/backend rules and ADDED the
...
- Next bring-up: B2 fixture pack, B3 `tests/design/` harness, B4 gallery. Then Batch 0 build
  (first credit spend â€” pause for founder go + capture the UI credit number first).

## 2026-07-24 â€” d86 re-flow on the design side (commit e165c6a)
...
- **Change order 003** delivered to the design chat: the one Batch 0 residual
  (coordination-components "asked on the task" â†’ "asked about"). Batch 5 anchoring is now
  carried by the updated spec â€” no separate action when it's drawn.
...
## 2026-07-24 â€” stable test handles ratified (change order 005)
...
- The founder ratified the test-handle convention (`loop/bringup/testid-convention-draft.md`):
  ...
  Lovable regenerates markup on every message â€” so tests need a contract that survives
  regeneration.
- **Folded** into `design/ui-ux-instructions.md` as new **Â§5.1** (inside Â§5 Tech mapping, where
  buildability contracts live). **Gate scan added** to `design/ui-way-of-work.md` Â§3 step 2:
  presence, grammar, and rename-diff â€” scoped to screens drawn on or after ratification.
...
- **Delivered to the design project 2026-07-29** â€” five days after it was folded. The fold,
...
## 2026-07-29 â€” the two shell questions RATIFIED by the founder (in the design chat)

- Verified from the design project's own transcript: the founder answered both gate-report-002
  questions directly in the design conversation â€” **"My projects is fine"** (the tab stays;
  ...
  (the tab stays; a thin organisation-level page â€” general balance + how it auto-applies,
  Discovery credits remaining, Lovable purse, VAT invoices â€” gets designed in a later batch).
- **Consequence, deferred not forgotten:** the screen inventory grows 27 â†’ 29. The two new
  rows (the Balance & invoices page; the My projects list) enter `ui-ux-instructions.md` Â§11
  and the batch plan with the Batch 3 change order, where they belong â€” nothing about Batch 1
  depends on them.
