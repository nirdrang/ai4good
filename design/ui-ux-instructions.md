...
> **Updated 2026-07-31** to d89 — the Discovery fit decline is now DECLINE-THEN-REVIEW: it
> still lands immediately and reads as final (no waiting state, no SLA anywhere in the NGO's
> view), but every decline opens exactly one platform-admin review item (uphold / overturn),
> the decline is a durable record on the project (never chat-only), and an admin overturn is
> the lifecycle's ONLY reverse transition — it reopens the same Discovery conversation.
...
- **Fit decline is a transition to `cancelled`, not a state (d89):** the declined project
...
| # | Screen | Purpose & key elements | States | REQ |
|---|--------|------------------------|--------|-----|
| 10 | **NGO dashboard** | Per project: status badge, % complete (done top-priority PM requirements / all — d82), fuel chip, Lovable chip, volunteer, cadence signals (last commit, requirements done/total, current requirement). Cross-project fuel summary; **general balance** as redeployable credit; action-needed rail (blockers by severity/age, scope-addition discussions, funding deadlines); a fit-declined project remains in the complete list with its decline record (cause, date, reshaping suggestion — d89) | empty (no projects) | REQ-013 |
...
| 22 | Notifications center | In-app inbox; read/unread; low-tone vs escalation tier; email mirrors critical events (money, deadlines, blockers, completion, decisions). Taxonomy in REQ-016; d89 adds three rows: **fit declined → NGO** (email + in-app; cause, reshaping suggestion, oversight sentence) · **fit-decline review → platform admin** (email + in-app; opens the ops item) · **decline overturned → NGO** (email + in-app; Discovery reopened). An UPHELD disposition sends the NGO nothing — the decline already stands | empty | REQ-016 |
