...
THE TEST SEAM AND THE TWO TIERS. Read `tests/at/suites/req-016/_contract.ts`, `_bind.ts`, `_fixture.ts`, `_live.ts`, `_integration.ts` and `_oracles.ts`. Explain the seam the suite expects a system under test to expose, and how the same test body runs against an in-memory binding at the loop tier and the real stack above it. Say how a tier chooses its procedure, how an id is declared red by capability name rather than by failure, and what `--expect` actually compares. Read `tests/at/expected/req-016.json` and say what its shape asserts. Explain how the live adapter reaches the database and the mail catcher, and how a world scopes its reads.
