...
THE DATABASE LAYER. Read every migration under `supabase/migrations/` whose name mentions notifications, in order. Map every table it creates, every column and constraint, every index and unique constraint, every SECURITY DEFINER function, every trigger, every sequence, and every grant, revoke and row-level-security policy. Say which tables are reachable by which client roles and which are unreachable, and how that posture is checked. Trace what one call to the emitter function actually writes, in what order, inside what transaction boundary. Read the fixture producer function and say exactly what its fault argument does and what survives a rollback. State how these tables join the tree's existing catalog scans.
