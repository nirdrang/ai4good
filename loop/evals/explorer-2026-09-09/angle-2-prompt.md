...
THE APPLICATION LAYER. Read `supabase/functions/_shared/notifications.ts`, `notification-taxonomy.ts`, `notification-copy.ts` and `notification-provider.ts`. Map the ports and the core: what the emitter decides, what the worker pass does, how a delivery moves between states, and where the provider is actually called. Read the SMTP provider implementation and say what protocol work it does itself and what it delegates. Say how the taxonomy table is validated and when. Say which module is allowed to import a provider client and how that rule is enforced. Trace one event from the emit call to a delivery row marked sent, naming every function on the path.
