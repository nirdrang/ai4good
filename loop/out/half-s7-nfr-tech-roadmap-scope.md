...
- p95: marketplace page < 500ms (re-validate pre-launch, post-migration); Discovery first token < 1.5s; payment webhook < 2s; code-hosting webhook < 5s.
...
- Provider keys (payment, LLM, code-hosting, work-tracking): managed secrets, never logged. Real LLM key never issued to volunteers; individual revocable credentials (decision-21).
...
- Per-project workspaces gone (decision-21); cap never binds. Org-level rate/quota limits monitored.
...
- WCAG 2.1 AA; full keyboard navigation; automated checks per change + manual screen-reader pass pre-launch; contrast >= 4.5:1.

### Compatibility

- Last 2 versions: Chrome, Firefox, Safari, Edge. Responsive web; no native apps in v1. English only at launch (→ RM-46).
...
Lovable-owned frontend; Supabase backend; Stripe fuel payments; GitHub App (repos, commit/PR ingest); Linear = task system of record; Anthropic = LLM. Volunteer builds reach Anthropic only via the LLM gateway on per-project virtual keys (REQ-009, decision-21 — caps, fuel gate, governance injection, metering: see Out of Scope §1); gateway alone holds the real key; hosting open (OD-6). Platform AI (Discovery Agent + assistant REQ-033, both Opus, decision-13) calls Anthropic directly. Volunteer LLM and task paths disjoint; sole touchpoint: task-ID binding (REQ-034). One platform-owned Linear workspace per project (decision-20), never transferred at handoff; NGO panel = read-only mirror; status moves on PR merge; out-of-band edits auto-reverted with comment (REQ-026). GitHub Issues dev-internal in v1 (REQ-008). Hard dependencies with defined degraded states: Supabase, Stripe, Anthropic, gateway, GitHub; Linear outage only stales the panel — mirror catches up. Greenfield; no migration.
...
- Attachments: Cloudflare R2 for NGO files (REQ-032); short-TTL signed URLs after membership check only.
...
- CI/CD: GitHub Actions (test/lint); frontend via Lovable, backend via Supabase CLI.
...
- **Phase 2 — Core MVP (weeks 4-7):** One project end-to-end: intake → credit-metered Discovery → editable scope → publish → triage screener → public listing → concierge enforce-match + volunteer consent (decision-28) → **fund-on-match** (not fund-on-publish) → repo + workspace provisioning → PRD bootstrap task + completion gate (decision-25) → fuel-metered build with fuel-low blockers → live-URL deploy → handoff with attribution. Exit: real NGO + volunteer reach a deployed live URL; handoff ritual executed.
- **Phase 3 — Dashboards, Comms & Platform Operations (weeks 8-10):** v1-minimal NGO/volunteer dashboards, comment thread, notifications, admin audit viewer, observability + alerting, incident response, automatic ledger reconciliation (decision-31), content moderation, abandonment/rematch; v1.5 items (§11) out. Exit: 3 pilot NGOs + 5 pilot volunteers fully in-platform (no Slack/email); injected money error auto-corrects, books balanced.
...
- **Phase 5 — Public Beta & Concierge Launch (weeks 14-16):** Launch checklist, invite-only → open rollout, internal goals analytics, concierge supply-funnel tooling, aging-open-project nudges, hand-match tool (first ~10-15 projects), v1.5 spec + backlog + retrospective. Exit: 10+ projects in flight; ≥1 public-beta handoff; ≥1 handoff reachable at its live URL 30 days later.
...
14. **ai4good-funded Lovable infrastructure** — NGO owns/pays its Lovable workspace; never billed against fuel; platform does meter/cap per-task usage and surface the NGO's balance (REQ-021/028).
15. **Multi-tool fuel metering** — fuel = Anthropic (Claude Code) only; other tools NGO-direct or volunteer-personal absent metering-compatible APIs.
16. **Lovable credit reselling through ai4good** — NGOs buy from Lovable directly; deep-linked "Top up" CTA only (→ RM-28).
17. **Service-level agreements / completion guarantees** — none: ghosting, fuel burn sans deliverable, infeasible scopes possible; platform bounds financial risk (per-project fuel caps), surfaces stalls, never underwrites.
...
25. **Automated PII / secret pre-scan on uploaded reference files (REQ-032)** — v1 = governance-by-disclosure: NGO acknowledges the data-responsibility rule; no scan (→ RM-37).
