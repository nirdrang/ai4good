...
**Why now:** (1) AI coding inflection — a single developer ships production software in days; (2) token-metered APIs: per-project dollar budgeting; (3) GitHub-native norms: free, transparent PM spine; (4) Stripe primitives: a credit ledger feasible for a small team.
...
Plain-language need → buildable scope that gets funded and built into a real tool; no technical spec. (Depends: REQ-001, REQ-002, REQ-004, REQ-005.)
...
- Per-request metering deducts from fuel — enforcement structural at the gateway (virtual keys, caps, inline metering, fuel gate), never polling-based: below 20%, NGO top-up prompt (REQ-024); below 5%, in-app dev warning; at 0, the next request is declined (fuel-suspension notice, both parties notified); top-up restores service, no reactivation step (REQ-009).
