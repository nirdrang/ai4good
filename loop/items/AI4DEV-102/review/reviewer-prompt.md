...
> This branch builds the founder's manual vetting action for an organisation and its complete audit
> record, plus everything the requirement attaches to that action. A platform administrator vets or
> unvets an organisation through one edge route and one security-definer function, with named
> evidence, an append-only audit row, and one notification for both outcomes. An organisation
> profile can be created and edited by its own administrator. A daily Discovery allowance is granted
> by tier, ten credits unverified and thirty vetted, spent against a per-day ledger row, reset at the
> UTC day boundary with no rollover, and blocked at zero with three remedies named. Vetting raises
> the cap in the middle of a day without refunding what was already spent, and unvetting never takes
> back credits the organisation already holds today. Vetting gates publishing and nothing else: it
> never gates Discovery and never gates funding, and email verification is the floor under every
> Discovery message. Several acceptance ids stay deliberately red because the surfaces they need,
> the publish flow, the triage queue, the Stripe project-fuel checkout, the funded-turn billing, a
> Discovery screen and the public listing screens, do not exist in this tree; the decisions those
> ids will consult ship as pure modules with no route behind them, and static arms assert the
> absence.
...
The repository is at the path given as your working directory. The change is the commit range
`main...426f2c1` on the branch `nirdrang/ai4dev-102-the-vetting-action-and-its-audit-record-d3`.

Read it yourself. The diff is about 17,000 lines across 68 files, so do not read it in one piece.
Start with `git diff --stat main...426f2c1`, choose what matters, and then read the changed files in
full rather than only the changed lines. A diff hunk without the function around it is the worst
thing to judge from.

The author has deliberately not told you where to look or what they suspect.

Useful orientation, all of it neutral fact:

- The product code is under `supabase/functions/_shared/` and `supabase/migrations/`.
- The tests are under `tests/at/`. `tests/at/suites/req-002/` is this requirement's suite;
  `_contract.ts` declares the system under test, `_fixture.ts` is the in-memory adapter, `_live.ts`
  drives the real local stack, `_source-scan.ts` holds static arms, and files ending `.test.ts` hold
  the acceptance bodies. `tests/at/harness/` holds the runner and the oracles for the arms.
- `tests/at/expected/req-002.json` declares which acceptance ids are green and which are red, per
  tier, and a run that does not match it exactly fails.
- `loop/items/AI4DEV-102/design/SYNTHESIS.md` is the design of record for this change.
- `CLAUDE.md` holds the project's standing rules.
...
relevant. Do not force lenses that don't apply.
...
1. **Severity**: `critical` | `warning` | `nit`
   ...
   - `nit`: Style, naming, minor improvement. Only include nits if they're genuinely useful.
...
Change nothing. You are read-only. Report only.
