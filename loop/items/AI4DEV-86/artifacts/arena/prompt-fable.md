...
YOUR DIRECTION: subtract first. Delete the no-target overloads of resetLocalDatabase and every personal-block constant and refusal; make CliTarget-plus-proof the only way to reach a destructive act; keep the identity read as a section of runner.ts (no new module) that generalizes the two container-name helpers by project id and workdir. Show the smallest surface that satisfies the constraints and what the deletions remove from runner.selftest.ts.
...
## Where to write
Write the package to loop/items/AI4DEV-86/artifacts/arena/candidate-fable.md (create it; the one file you may write). Reply with a three-line summary only.