...
YOUR DIRECTION: one new module, tests/at/harness/local-stack.ts, that owns everything the one stack needs (identity read, lock, reset with proof, migration proof, attestation write, child env, evidence line) behind ONE function the runner calls from its integration branch, returning env plus evidence plus the lock to release. The runner keeps only its target-neutral primitives and imports the module instead of the pool. Show the module public surface and why it is no larger than needed.
...
## Where to write
You are in a read-only sandbox. Return the complete design package as your final answer; do not write files.