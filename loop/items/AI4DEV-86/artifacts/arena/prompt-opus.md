Read loop/items/AI4DEV-86/artifacts/arena/task.md in full and produce the design package it asks for.

YOUR DIRECTION: keep the proven read. Move the pool functions that are not slot-specific in substance (proveSlotTarget generalized to proveLocalTarget, ownContainerNames, foreignContainerNames, slotDbContainers as the docker probe) into a small tests/at/harness/stack-identity.ts keyed by project id and workdir, keeping the destructive-path rule that a reset needs a positive container name AND a docker ps match; the runner composes it with its own lock, reset, attestation, and env. Show why the docker probe earns its place on the one stack and what it costs when Docker is absent.

## Rationale template (shape your package on this)
...
## Where to write
Write the package to loop/items/AI4DEV-86/artifacts/arena/candidate-opus.md (create it; the one file you may write). Reply with a three-line summary only.