...
YOUR DIRECTION: repoint in place, smallest diff. No new module. Replace the occupy/prepare/evidence/stackEnv calls in the runner integration branch with an inline sequence over the runner primitives that already exist (acquireStackLock, readStackStatus, localStackProblems, waitForReady, resetLocalDatabase(target, proof), proveMigrationsReplayed, writeAttestation, childEnv), moving only the two container-name helpers into runner.ts. Count every line you add and remove.
