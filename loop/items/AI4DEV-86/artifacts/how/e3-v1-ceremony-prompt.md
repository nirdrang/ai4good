...
The v1 ceremony outside the tests. Inventory .claude/agents/*.md (conductor, orchestrator, orchestrator-opus, executor, reviewer-runner, distiller, mechanical): what each is for, and which files or scripts reference each by name (grep the repo, including .claude/skills, loop/, .github/, CLAUDE.md). Inventory every script in loop/work/*.ps1 and loop/work/attribution-epoch.txt: what it does, and who calls it: .claude/settings.json (hooks, statusLine), .github/workflows/ci.yml (the twin-guard step calls loop/work/twin-check.ps1), other scripts (dot-sourcing work-lib.ps1), skills. Produce a dependency table: script -> callers. Say which of banner.ps1, ci-status.ps1, context-gauge.ps1, statusline.ps1, guard-branch-switch.ps1, sheet-check.ps1, render-mermaid.ps1, pstack-models.expected.md are still referenced by live config, and which of twin-check, stamp-hook, attribution-report (+selftest, epoch), watch-items, work-lib, materialize, db-slots have no live caller once the twin-guard CI step is dropped. Also read .claude/skills/work/SKILL.md, WORKFLOW.md, shared-invariants.md headers to say what the old /work prose is and what the three standing rules are, and CLAUDE.md section 5.
