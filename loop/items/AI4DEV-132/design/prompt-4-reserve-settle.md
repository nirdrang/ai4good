...
## Your structural direction: two-phase turns, reserve then settle

Treat a turn as two writes. Preflight reserves credits (or fuel) for the turn's estimated cost before the model call, with the reservation a row that bounds spend; the model runs; settle records the actual usage and releases or confirms the reservation. Overspend is impossible because the reservation caps what the model call may consume (a max_tokens derived from the reservation), and a crash between the phases leaves a reservation that a later turn or a sweep settles. Funding is a target on the reservation. The conversation store is whatever you choose, but the reservation and settle rows are the turn records the transparency read exposes. Push this direction: is a two-phase turn worth its second write at this scale, and how does the loop tier prove the crash case? Say where it breaks.
