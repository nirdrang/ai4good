...
## Your structural direction: one pure billing kernel in TypeScript

Put every decision in one pure `_shared` module, `discovery-billing.ts` (or your name): gate order, email floor, kill switch, funding route, allowance preflight with the ratio and rounding, the remedy set by tier, and the settle computation after the model returns. It takes a typed snapshot of the world (standing, vetted, email confirmed, kill switch, funding state, today's spend, the model's usage) and returns a typed decision (refuse with kind and remedies, or admit with target and credits). SQL definers are thin appliers that re-check the invariants they can (spend bound, kill switch, membership) and write. The loop fixture drives the kernel with in-memory state, which is where most of the twenty ids go green. Push this direction: how thin can SQL be while `spent <= granted` and the funded-never-pool rule stay enforced in the database? Say where a pure kernel cannot hold an invariant and the database must.
