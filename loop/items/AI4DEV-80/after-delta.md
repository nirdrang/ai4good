# after-delta.md — what the mechanism changed, measured at the goal head

Written by the EXECUTOR of the fix-and-goal pass, 2026-08-11, at the goal head of
AI4DEV-80 (attribution by spawn tree). Every number here is RE-MEASURED at this head. The
draft sitting's numbers are quoted only for comparison, and they differ — the reason is
stated below rather than smoothed away.
...
- before: `report-before.txt` and `report-before-79.txt`, captured at S1 before any code
  changed;
- after: `report-after.txt` and `report-after-79.txt`, captured at this goal head.

## 1. The headline, with BOTH denominators

The two percentages do not divide the same total, so neither is readable alone.

| | before (S1) | after (goal head) |
|---|---|---|
| transcript files scanned | 479 | 919 |
| responses counted | 26352 | 49071 |
| output tokens counted | 39659466 | 49963158 |
| unattributed output tokens | 27986120 | 33792045 |
| **unattributed share** | **70.6%** | **67.6%** |

The after share is strictly lower, by 3.0 points. That is the whole movement in the
...
**The absolute unattributed number went UP**, from 27.99 million output tokens to 33.79
million. That is not a regression. 440 more transcript files are counted at all, and the
...
| | before | after | change |
|---|---|---|---|
| attributed output tokens | 11673346 | 16171113 | +4497767, +38.5% |
| attributed responses | 10816 | 21085 | +10269, +95.0% |
...
| the spawn tree | 14 | 4188 | 1940941 |
| the session stamp | 11 | 211 | 224337 |

The `tree` source did not exist before this item. It attributes 4188 responses across 14
...
table now shows the whole relay, which is what ruled bullet 3 asked for.

## 4. The floors, restated at this head

- **Ambiguous agents: 2.** Two agent transcripts name two or more items in their own
  records. Their branchless responses stay unattributed, and — new at this fix pass, ruling
...
## 5. Why these numbers differ from the draft sitting's

The draft sitting measured 70.6% → 67.7%, files 480 → 912, responses 26371 → 48658. This
head measures 70.6% → 67.6%, files 479 → 919, responses 26352 → 49071.

The store is LIVE. It grows while the item is worked, because the sittings and executors
working this item write their own transcripts into the very store the report reads. So a
re-measurement can never reproduce an earlier one exactly, and the direction of the drift is
always more files and more responses. The two `before` captures in this record differ from
each other for the same reason: `report-before.txt` saw 479 files and `report-before-79.txt`,
run seconds later, saw 480.
...
comparison in this note is drawn from the committed pair above.

The section 2 breakdown table comes from a `-Json` run a few seconds after the committed
capture, which saw 49074 responses rather than 49071 — three responses of drift, stated
rather than hidden.
