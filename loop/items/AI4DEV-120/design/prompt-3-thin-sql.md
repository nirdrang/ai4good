...
## Your structural direction: thin SQL, one route, TypeScript owns the decisions

Plain tables with the posture and nothing more. One edge route for the whole intake, with a verb field in the body (`start`, `patch`, `attach`, `submit`), and one pure decision module under `supabase/functions/_shared/` that holds every gate as a table or state machine: the admin gate, the description gate isolated from verification and capacity, the disclosure selection by classification, the transition. SQL definers are few and generic, and `decide` does the judging. The suite's loop tier runs the same decision module over Maps. Say how the SQL side still protects the invariants the policy scan demands (assert_account_active, row lock, organisation re-check), and where a single route with a verb field beats or loses to one route per verb.
