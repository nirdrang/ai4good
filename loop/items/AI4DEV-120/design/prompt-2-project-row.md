...
## Your structural direction: the need is the project, from day one

A project begins life as a need, so the `public.projects` row is created at intake and carries the need's identity. Because a state column cannot sit on a table named like a project, and because `public-project` reads every projects row anonymously, the intake fields, the stage, the classification and the snapshot live on a 1:1 companion table keyed by `project_id`, with the name you choose. Argue for this shape: the single-developer seat, the tenant read posture and the dashboard `projectsOf` all already work on `projects`, so a need that is a project row inherits them. Say what the anonymous public read must do about a draft, how `has_platform_acknowledgment` fires at creation as the projects table comment demands, and where this shape breaks if it does.
