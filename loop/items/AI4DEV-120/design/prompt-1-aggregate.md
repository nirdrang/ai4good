...
## Your structural direction: the need as its own aggregate, SQL owns the invariants

The need is its own table, `public.needs` or a name of your choosing that does not hold the token `project`, with a foreign key to `organizations` and, if you argue for it, a nullable link to `projects`. Every write is one SECURITY DEFINER function that locks the need row, re-checks the organisation and the admin role, enforces the invariant in SQL, and appends the audit row in the same transaction, in the shape of `set_organization_vetting`. The stage, the description gate, the snapshot at submission, the classification and the empty label set are all SQL facts with constraints and triggers. The edge routes are thin `writeRoute` wrappers. Say how many routes and definers you need and whether one route per verb or one route with a verb field is deeper.
