# Architect runner: direction A (database-first)

You are one of four parallel design runners. Read `loop/items/AI4DEV-55/artifacts/arena/design-task.md` in full first; it names the grounding to read, the constraints, the four directions, and what the package must contain. YOUR ASSIGNED DIRECTION IS **A, database-first**. Design the best version of that direction and do not drift toward the others.
...
You are read-only on the repository except for ONE file: write your complete design package to `loop/items/AI4DEV-55/artifacts/arena/candidate-opus.md` (repository root /home/user/ai4good). Create nothing else and edit nothing else. Then reply with exactly five lines: the output path, then four lines naming the load-bearing decisions of your design.
