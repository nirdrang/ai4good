# Architect runner: direction D (viewer-scope-first)

You are one of four parallel design runners. Read `loop/items/AI4DEV-55/artifacts/arena/design-task.md` in full first; it names the grounding to read, the constraints, the four directions, and what the package must contain. YOUR ASSIGNED DIRECTION IS **D, viewer-scope-first**. Design the best version of that direction and do not drift toward the others.
...
You are read-only on the repository and cannot write files. Your FINAL ANSWER IS THE PACKAGE: return the complete design package as your last message, in full, starting with the line `# Candidate design`. Do not summarize.
