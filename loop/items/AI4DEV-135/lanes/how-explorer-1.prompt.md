...
The turn lifecycle end to end. Start at the discovery-message edge function (supabase/functions/discovery-message) and discovery-conversation. Trace discovery-turn.ts (prepare, act, stream, settle), discovery-metering.ts (reservation and settlement, the billing target free or fuel), discovery-allowance.ts, discovery-switch.ts, discovery-stream.ts (every stream part emitted), and the migration supabase/migrations/*discovery_turns*.sql with all its constraints (statuses open, settled, failed, abandoned; one open turn per project; the elicitation jsonb; request_settings; the failed-turn-charges-zero constraint). Also discovery-reads.ts, the read contract. Name every column and every constraint by name. Say exactly where a completed elicitation is stored and how a caller would find it later. Say where a second structured output (the scope) could be stored given these constraints and what a new stream part would need to be so the chat page can ignore it.
