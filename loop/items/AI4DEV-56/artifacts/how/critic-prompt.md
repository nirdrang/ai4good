...
You are read-only. Do not modify any file, do not run git, do not start any process or container.
...
- **`LEAF` / `notLanded`** (`tests/at/suites/req-001/_pending.ts` lines 47 to 90). The map of leaves that still own stubbed ids, and the stub body that throws `AtPending(ctx.atId, 'sut-missing', â€¦)`.
...
3. `edgeHandler('complete-signup', â€¦)` (`edge.ts` lines 101 to 114) answers OPTIONS with 204, maps thrown errors to a shaped 502, and runs the inner handler. CORS allows POST and OPTIONS only (`edge.ts` lines 72 to 76).
...
8. `callDatabaseFunction(url, serviceRoleKey, 'complete_signup', { p_account_id: caller.id, â€¦ })` posts to PostgREST as the `service_role`.
...
`bun run at:check req-001` (`tests/at/harness/check.ts`) reads every `(P0)` id from `.taskmaster/docs/acceptance/at-req-001.md` and every `atTest('AT-â€¦'` call site under `tests/at/suites/req-001/*.test.ts`, and exits 0 only on an exact bijection. Thirty-seven and thirty-seven today. It does not read the expected JSON.
...
`atTest` (`registry.ts` line 861) is the only place an id becomes an `it()`. Suite files import it from `_bind.ts`, which is `bindSuite({ requirement: 'req-001', sut: 'accounts', â€¦ })`. A body is one function for every tier or a per-tier map that must name every tier or `default`; a hole makes the id MISSING, which no declaration can describe. The body receives `ctx.open()` and gets `{ h, w, sut }` where `sut` is already the bound `AccountsSut`. A body that never opens is INVALID unless it throws `AtPending` or `CapabilityPending` first.
...
In that same change land `atTest('AT-001.41', â€¦)` in a suite file, the expected JSON entry at both tiers, and no new `LEAF` key if the id is green. Three independent bijections must hold at once. Acceptance file to decomp (`check-tree.ps1`), acceptance file to call sites (`at:check`), acceptance file to expected JSON (`--expect`). Recall that the integration body will need the identity row id from a live `/user` body and an HTTP DELETE, which no helper in `live-stack.ts` provides today.
...
Paths are relative to the working directory, which is the repository worktree.
...
loop/items/AI4DEV-56/artifacts/measure/unit4-privileges-after-reset.txt
loop/items/AI4DEV-56/artifacts/measure/unit6-signin-rate-limit.txt
loop/items/AI4DEV-56/artifacts/measure/unit6-auth-container-env.txt
...
## The work this critique serves

The reader of your findings is about to DESIGN six units on top of this subsystem (the brief is loop/items/AI4DEV-56/brief.md; read it): an audited platform-admin-only contact transfer and lost-access recovery with an escalation contact; a lifecycle state that gates every write through one registered boundary with a conformance check; an append-only audit for role changes and contact transfer plus a sign-in rate limit; a proof about leftover table privileges; a server-side refusal of GitHub identity unlink for volunteers; and a record of what the local auth rate limit honours. Measurements already taken are under loop/items/AI4DEV-56/artifacts/measure/ (read them): the privilege state after reset, the unthrottled sign-in probe, the auth container env, the ban probe (a ban does not end a live access token), and the hook push probe (the CLI does push the password-verification hook). Weight your findings toward what would make those six units wrong, brittle, or double-stated if built on the current structure as it stands.
...
Write your whole critique as your final answer in this shape:
