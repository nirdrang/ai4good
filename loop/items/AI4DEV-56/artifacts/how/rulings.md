...
**R16. The transfer moves the seat and deactivates only a seatless account (supersedes R6).** (interrogate: fable 1, grok 1, opus 1.) R5 makes every transferee hold its own seat, so R6's refusal made every transferee permanently untransferable after one use. The transfer now moves the named seat; if the outgoing account then holds no other seat it is deactivated, otherwise it stays active and the transfer's audit row records the remaining seats. holds-other-seats leaves the wire. AT-001.25 proves the second step: the seat moves on from the transferee to a third account.
