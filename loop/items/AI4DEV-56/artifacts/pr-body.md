...
- The `verify-ai4good` skill gains two things this run needed: its live drive normalises Docker Desktop's WSL bind-source form, so the mount check passes on this host, and its Drive section forbids pasting `db:start` output into a committed file, because that output prints the local stack's keys and GitHub push protection then refuses the whole branch.
