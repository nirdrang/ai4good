...
- `supabase/functions/_shared/admin-operations.ts` (new) holds `decideContactTransfer`, `decideEscalationContact`, `decideLifecycleChange`; `accounts.ts` gains `AccountLifecycle`, `decideSignupCompletion`, `decideOrganizationCreation`; `memberships.ts` gains `decideOrganizationRename`; `gateway-keys.ts` (new) holds `virtualKeyActionFor`, the seam the LLM-gateway requirement must consult.
- Four migrations: `20260908120000` (the `account_lifecycle` enum and column on `accounts`, the `audit_events` and `org_escalation_contacts` tables unreachable by every client role, the append-only row and statement triggers, `assert_account_active` with `for share`, `write_standing`, `append_audit_event`, `change_account_lifecycle`, `transfer_organization_contact`, `set_escalation_contact`, and `create_organization` and `update_organization` recreated with the gate first); `20260909120000` (`set_account_lifecycle`); `20260910120000` (the role-change trigger on `org_memberships`); `20260911120000` (the `before delete` trigger on `auth.identities` guarded by `when (pg_trigger_depth() = 0)`).
...
- The transfer refuses when the outgoing account holds any other seat and names the other organisations, because lifecycle is account-level and deactivating the account would gate its writes in organisations the transfer never looked at.
- The transferee is an existing completed NGO account; its own organisation from signup stays with it.
- Product membership inserts (`complete_signup`, `create_organization`) record the audit actor as `operator`, because those definers set no actor and this branch does not recreate them a second time. The transfer records the administrator.
- The virtual-key clauses of AT-001.30 and AT-001.31 are declared (`gateway.virtual-key-revocation`, `gateway.virtual-key-reissue`) rather than stubbed with a table nobody reads. Building the stub is one more commit group if the founder wants it.
...
- The audit actor on product membership inserts (`complete_signup`, `create_organization` set none).
