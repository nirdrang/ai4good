# Arena runner: codex:gpt-6-astra@medium
...
SQL FIRST. Every rule of this subtree is a database object: the lifecycle state, the gate helper called first in every write definer, the transfer as one definer, the audit as a table with its own triggers, the escalation contact as a table. TypeScript is a thin twin: the shared module holds only the readable refusal for the loop tier, and the edge routes stay as they are plus one call. The write inventory is DERIVED from the migrations (which definers are granted to service_role and are not stable), and the conformance scan is an extension of _policy-scan.ts over definer text. Make the SQL side the source of truth and say what the TypeScript twin may never decide on its own.

## Output

Your sandbox is read-only, so you cannot write any file. Deliver the WHOLE design package as your final message: the launcher saves your final message, and only your final message, to loop/items/AI4DEV-56/artifacts/arena/candidate-astra.md. Anything you say in an earlier message is lost. Put the five-line reply at the very end of that same final message under the heading ## Five lines. Do not run git, do not start processes, do not touch the database.
