...
/** Narrow two service-role rows into a CallerAccount, or null when there is no account row. Unknown type or lifecycle → null (refused). */
...
export type WriteRouteSpec = {
   ...
   *   'none'   — the body names no organisation
   *   'exists' — the organisation must exist; the caller's role there is not consulted (admin routes)
   *   'admin'  — the caller must hold the admin role there (orgAdminActionAllowed decides)
   */
...
// active→deactivated: 'revoke'; deactivated→active: 'reissue'; same→same: null
```
...
```
request ──► resolveCaller (edge.ts, I/O) ──► Caller | null
        ──► loadCallerAccount (edge.ts, I/O): one service-role read of accounts, one of org_memberships when route.organization !== 'none'
        ──► callerAccountFromRows + parseOrgRole (pure)
        ──► writeAllowed(route, account, orgRole) (pure) ──► refusal { kind, reason } at WRITE_REFUSAL_STATUS[kind]
        ──► parse(body) (pure, per route) ──► 400 { kind: 'invalid-request', reason }
        ──► callDatabaseFunction(route.definer, args(request, account)) (I/O)
              └─► definer: perform public.assert_account_active(p_actor) FIRST (share lock, R7), then its own backstops, then writes
        ──► definerRefusal(outcome) (pure): 4xx → 409 { kind: <from DETAIL> | 'refused', reason }; 5xx/transport → 502
```
...
| AT-001.29 | per type in `ACCOUNT_TYPES`: an active control and a subject, both otherwise authorised for each route in `routesAdmitting(type)`; subject deactivated via `setAccountLifecycle(A, …)`; scan `[]` | each route's write for control then subject | control `ok`; subject `{ ok: false, kind: 'account-deactivated' }`; read-back shows no write; `writeRouteProblems()` equals `[]` | green over all three arms (volunteer via the stand-in) | NGO and admin arms real, then `CapabilityPending(['sut.accounts.sendDiscoveryMessage'])` (R2) |
...
The transfer, row by row: `org_memberships` — the one row for O changes `account_id` from X to Y, keeps `role = 'admin'` and `created_at`; `accounts` — X gains `lifecycle = 'deactivated'`, Y unchanged; `acknowledgments` — no row touched, X's signer name, title and attestation stay X's; `projects` — no row touched, `org_id` still O, the volunteer seat untouched; `volunteer_profiles` — never touched by an NGO transfer; `audit_events` — two rows appended. Y's own organisation from signup stays seated by Y (R5 residual). X's Auth user is not deleted, banned or logged out (R8; "Not done here").
