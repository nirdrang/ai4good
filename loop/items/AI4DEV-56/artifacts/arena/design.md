# Design: the boundary is the handler

The arena's synthesized design. Base: the opus candidate (`candidate-opus.md`); grafts from the astra, fable and grok candidates are folded in where they land and listed under Synthesis decision. Structural claim: **a `writeRoute()` constructor owns the whole write pipeline, and a route registers by being constructed through it.** One design for the six units of `loop/items/AI4DEV-56/brief.md`.
...
1. **`service_role` cannot write any public table.** After the privilege overlay it holds `SELECT` on
   ...
   any running service — and it is the layer CI never runs, because CI runs the loop tier only.
...
### Call site 1 — the transfer route, whole
...
### Call site 2 — one gated write, after the change

```ts
// supabase/functions/update-organization/index.ts   (was 137 lines; the role lookup and the
// judgement both move out, and nothing is lost — `roleIn` becomes the shared standing loader and
...
```
POST /functions/v1/update-organization   { "organizationId": "…", "name": "Riverside Shelter" }
200 { "ok": true, "organizationId": "…", "name": "Riverside Shelter" }
403 { "ok": false, "kind": "not-an-admin",         "reason": "…" }     ← unchanged
403 { "ok": false, "kind": "account-deactivated",  "reason": "…" }     ← new, from the gate
```
...
export async function at00141(ctx: Ctx): Promise<void> {
  ...
  // THE CONTROL. An NGO account's GitHub identity is not mandatory, so the same call succeeds —
  ...
  await sut.completeSignup(ngo, { accountType: 'ngo', organizationName: 'Riverside Shelter 41', … }, CLIENT_IP);
...
`accounts` carries the state and nothing else — no `deactivated_at`, no `deactivation_reason`. When
...
export function parseWriteStanding(raw: unknown): WriteStanding;   // fail-closed; unknown → 'unreadable'
```
...
create table public.audit_events (
  ...
  subject_org_id    uuid,
  reason            text not null,           -- every row says why; the trigger writes its own sentence
...
```ts
// supabase/functions/_shared/write-routes.ts — the PURE spine, graded at the loop tier
...
1. `unreadable` → `refused`, 502. Never a judgement about the caller.
2. `lifecycle === 'deactivated'` → `account-deactivated`, 403. **Before** the type check and before
...
3. Route admits an absent account → admitted. `complete-signup` reaches here, and note that step 2
...
4. `no-account` → `no-account`, 409.
5. Type not in `admits` → `typeRefusalKind(admits)`, 403.
...
/** RpcOutcome gains `details: string | null` (PostgREST forwards DETAIL); the pure `definerRefusalKind(details)` maps a closed kind and fails closed to 'refused'. */
```
...
— nine steps and about 120 lines that every route used to restate in part. What stays exposed to the
...
tier therefore grades the shipped gate, the shipped decisions and the shipped inventory — not a copy
 ...
 *  empty functions directory — an absence reported by a broken instrument is the false green this
...
| `write-route-registered-under-other-name` | a registered route whose `name:` literal is not its own directory name, so a copied file registered under another row fails |
| `stand-in-not-gated` | a `stand-in` row whose name never appears in `tests/at/suites/req-001/_fixture.ts` as `writeGateDecision('<name>'` |
...
`surface.kind === 'edge'`, **and** the file's only server is `Deno.serve(writeRoute({ … }))`, **and**
...
**What this scan proves and what it does not.** It proves registration and construction — that every
...
it('fails a write route that reaches the database without registering', () => {
  const problems = scanWriteRoutes(WRITE_ROUTES, [
    ...realRouteFiles(),
    { name: 'donate-fuel/index.ts', text: `Deno.serve(async (r) => fetch(\`\${URL}/rest/v1/rpc/donate_fuel\`, …));` },
...
| `audit-mutation-in-definer` | any function body in the migrations that contains `update public.audit_events` or `delete from public.audit_events`; R3's second clause as a check, not prose |
...
(`20260907120000`), and a stamp is a key — a collision stops the replay, which this tree has already
paid for once.
...
-- R6's other-seats check and the standing loader's seat count query by account; the unique index is by organisation.
...
create table public.audit_events ( … as above … );
create table public.org_escalation_contacts ( … as above … );
...
create function public.assert_account_active(p_account_id uuid) returns void … ;   -- body above
...
begin
  ...
  -- reason: 'membership granted' on INSERT; 'seat repointed' when account_id changed; 'role changed' otherwise.
...
begin
  ...
  --    that the outgoing account holds no other seat (R6) — every one a backstop for a caller that
  ...
  --      where org_id = p_organization_id and account_id = p_from_account_id;   ← the seat moves
  -- 4. public.change_account_lifecycle(p_from_account_id, 'deactivated', p_account_id, p_reason)
  -- 5. public.append_audit_event('org_contact_transferred', …, p_reason, …)
  raise exception 'not implemented';
end;
$$;
...
create or replace function public.create_organization(p_account_id uuid, p_name text) … ;  -- + assert_account_active first
create or replace function public.update_organization(p_account_id uuid, p_organization_id uuid, p_name text) … ;
...
begin
  ...
  -- re-check platform_admin, then public.change_account_lifecycle(...) and return {changed: …}
  raise exception 'not implemented';
end;
$$;
...
-- is the row (R10). The depth discriminator sits in the trigger's WHEN clause, where a direct delete
-- sees depth 0 and a cascade from a vendor admin user delete sees depth 1 or more (R3); inside the
-- function body the depth is already 1, so a body test for 0 would never fire. The prototype
-- measures the cascade case before this lands.
create function public.github_identity_is_permanent_for_volunteers()
returns trigger language plpgsql security definer set search_path = ''
as $$
begin
  -- if old.provider = 'github' and the owning public.accounts row is of type 'volunteer', raise
  raise exception 'not implemented';
end;
$$;
...
The static scan reads all three files and must find, for every new table, a `revoke all … from anon,
...
|---|---|---|
| `org_memberships` | `account_id` moves from A to B on the single seat row; `role` is untouched | the shape `repointMembershipAsOperator` already proves live; the NGO-only trigger checks B, and the `after` trigger writes the `org_role_changed` audit row with reason `seat repointed` |
...
| `acknowledgments` | **nothing** | `account_id` still points at A — this is AT-001.25's "still attributed to the original acting humans"; the signer name, title and attestation stay exactly as A signed them |
...
| any write | the definer's backstop raised | 409 (4xx) / 502 (5xx) | the `kind` the definer put in `RAISE ... USING DETAIL` when it is one of the closed kinds, else `kind: "refused"`; the database's sentence as `reason` (R10) |
...
decision, `supabase/functions/_shared/gateway-keys.ts`, the hook the gateway leaf must consult —
...
| **.27** recovery | a second organisation whose contact cannot sign in | the **same** route, `reason: 'lost access — recovery'` | identical postconditions to .25 plus its own audit row; the body states that recovery is the same operation and the reason carries the story | G | G |
...
| **.35** admin only | an NGO account, a volunteer, no session, and an admin control | the transfer route, four times | NGO → 403 `not-a-platform-admin`; volunteer → the same; no session → 401; admin → 200; and after the three refusals the seat and both lifecycles are unchanged | G | G |
| **.29** every write | per admitted type: one active control and one deactivated account, otherwise authorised | iterate `WRITE_ROUTES`; for each row and each admitted type, `attemptWrite(route, session, subject)` twice | every deactivated attempt refuses with `account-deactivated`; every active control succeeds. Plus `writeRouteProblems()` is empty, at both tiers | G | **CP** `sut.accounts.sendDiscoveryMessage` (after the NGO and admin arms run for real) |
...
| **.34** sign-in limit | — | — | declared (R11): the local GoTrue answered 45 password grants in 0.66 s with zero 429 | **CP** `vendors.gotrue-sign-in-rate-limit` | **CP** same |
...
**New pure modules (3)** — all under `supabase/functions/_shared/`, all imported by the fixture, so
...
`discovery-message` entry throws `CapabilityPending(['sut.accounts.sendDiscoveryMessage'])`.

**New harness pieces (4)**: `_write-route-scan.ts`, `write-route-scan.selftest.ts` (10 cases),
...
`[auth.hook.password_verification_attempt]` into the container — measured — so a product-side counter
...
Base: the opus candidate, scored 18 of 18 by the lead and 17 by the cross-judge, which also picked it; the judge's verdict and the full scoring are in `synthesis.md`. Grafts folded in above: the trigger-depth discriminator in the unlink trigger's `WHEN` clause with the depth semantics stated (astra); an index on `org_memberships (account_id, org_id)` for the other-seats check (astra); the scan's own-name check and the stand-in row check (fable); the `audit-mutation-in-definer` scan code (fable); `reason` not null on every audit row (grok); the definer's refusal kind travels in `RAISE ... USING DETAIL` and a fail-closed pure parser maps it, so a backstop refusal keeps its kind on the wire (fable, raised by the judge under R10); the volunteer arm's capability string is `sut.accounts.sendDiscoveryMessage`, the one the live adapter already throws, not a new string (judge). Rejected: grok's one `admin-operations` dispatcher and audit-first projection; astra's generated inventory, request-id receipt and SQL twins for the stand-in rules; fable's `lifecycle_changed_at` column and its pure unlink twin. The reasons are in `synthesis.md`.
...
- **We accept one extra database round trip on the transfer path, in exchange for the transfer's
  judgement being pure and graded at loop.** The alternative — a definer that answers refusals as
  data — keeps the round trip count at two and puts the reasoning where CI cannot read it.
...
- **We accept `pg_trigger_depth()` in the trigger's `WHEN` clause as the discriminator for the unlink
  trigger**, which is a measurement we have not taken yet. If a cascade turns out to fire at depth 0, the trigger refuses a
...
- **We accept three admin routes rather than one `admin-operations` endpoint with an operation field.**
  ...
  inside a handler — the exact shape this design removes everywhere else.
...
services — it catches a raw service-role RPC, which the handler cannot — and it needs no inventory at
...
deactivation would take effect up to two minutes later — against a criterion whose word is
...
second statement of a rule only SQL enforces — the exact defect `accounts.ts`'s header exists to
...
7. **Risk: the standing loader is one more `stable` definer on the hot path of every write.** It
   replaces two ad-hoc reads, so the count does not grow — but it does put one more object in the
...
Write `supabase/functions/_shared/write-routes.ts` — the inventory, `WriteStanding` with its
fail-closed parser, and `writeGateDecision` with its five ordered checks — plus
