# Arena runner: grok:grok-4.6@xhigh
...
ADMIN OPERATIONS AS ONE FUNCTION AND THE AUDIT AS THE SPINE. One new edge function, admin-operations, dispatches the platform administrator acts (transfer, recover, deactivate, re-enable) as a closed vocabulary of typed commands; every act writes its audit row FIRST in the same transaction and the account and seat state follow from it. Role changes reach the same audit table by trigger. The lifecycle gate reads the state the audit produced. The inventory of writes is the command vocabulary plus the three existing routes. Make the audit table the one place a reviewer reads to know what happened, and keep everything else derivable from it.

## Output

Do not write any file. Deliver the WHOLE design package as your final message: the launcher saves your final message, and only your final message, to loop/items/AI4DEV-56/artifacts/arena/candidate-grok.md, and it overwrites anything written there earlier. Anything you say in an earlier message is lost. Put the five-line reply at the very end of that same final message under the heading ## Five lines. Do not run git, do not start processes, do not touch the database.
