# Arena runner: claude:opus@xhigh
...
THE BOUNDARY IS THE HANDLER. A writeRoute() constructor in supabase/functions/_shared/edge.ts owns the whole write pipeline: caller resolution, the caller-account load (type, lifecycle, target-organisation role), the lifecycle gate, the shared decision the route names, and the RPC. A write route REGISTERS by being constructed through writeRoute(); nothing else can call callDatabaseFunction. The conformance scan checks that every index.ts that imports callDatabaseFunction is exactly a Deno.serve(writeRoute({...})) with a name that appears in the inventory. Make the pipeline deep and the per-route code a few declarative lines, and state what the SQL side still backstops.

## Output

Write your design package to loop/items/AI4DEV-56/artifacts/arena/candidate-opus.md. You are read-only everywhere else: do not run git, do not start processes, do not touch the database, do not modify any other file.
