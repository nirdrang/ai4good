...
- `.claude/hooks/session-start-banner.sh` stays live and exits unless `CLAUDE_CODE_REMOTE` is true. With banner.ps1 parked, a local session has no session-start banner from either source. Recorded, not changed.
