...
- **Role model (verified against Lovable's role matrix, 2026-07-12).** Billing (Plans & credit usage — top-up, plan, payment method) is **owner/admin-only**: "all other roles see the tab but every billing control is disabled and cannot initiate or approve charges." So the volunteer is invited as **Editor**, not admin. An Editor **can** *Manage GitHub* (connect two-way sync) and *Publish*, plus build/edit/set project knowledge; an Editor **cannot** *Disconnect Supabase* (DB connection = admin/owner), *Manage Integrations* (workspace-level connectors = admin/owner), or access billing. So kickoff splits by tier:
  - **NGO owner (owner/admin-gated):** create workspace + plan + card; **enable the database**; add any workspace-level connectors; **set the volunteer's per-member `monthly_credit_limit`** (native Lovable spend cap, UI-only, no MCP setter); invite the volunteer as **Editor**.
  - **Volunteer (Editor):** provision the project, **connect GitHub** (repo → platform org), inject conventions + reviewer skill, build, publish, consume credits — never touches billing.
- `[verify]` member add/remove/self-remove via Lovable API/MCP; fallback = manual NGO removal.
- `[verify]` whether the **initial "enable Lovable Cloud database" toggle** is editor-blocked (the matrix confirms *Disconnect Supabase* = admin/owner; the initial enable is unconfirmed) and whether an **Editor can `create_project`**. If both are editor-allowed, the volunteer does the entire setup and the NGO's only technical step disappears; if not, they stay with the NGO owner. 5-minute test: invite a second account as Editor and check the enable-DB + create-project affordances.
