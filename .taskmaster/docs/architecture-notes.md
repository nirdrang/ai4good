...
- `[resolved — d47]` create-project + enable-DB are editor-allowed (matrix above), so the NGO's technical setup step disappears — the volunteer does it all.
- `[verify]` **credit-status read at Editor role.** *View usage* = Editor **No** in the matrix → the volunteer's editor session may be unable to read the workspace credit balance via `get_workspace`, which REQ-021/d42 assume. Live-check `get_workspace` as an editor; if blocked, the credit-status read moves to the NGO/owner session or the manual-report fallback (already in REQ-021).
