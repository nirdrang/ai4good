...
5. **Inline metering (demoted to preview + gate trigger).** The gateway's per-request estimate (local rate card) is now only a **live gauge and the immediate real-time zero-fuel decline** — never the ledger. The ledger truth is the provider's per-workspace billed cost; the 15% margin is recognized on that. So the earlier "fragile token math" no longer determines any dollar figure — at worst it trips the gate slightly early/late, then reconciles to the exact bill. Fuel warnings at 20% (NGO) / 5% (volunteer); the workspace spend limit is the hard backstop under the gateway's soft gate.
...
- **Level 2 — inline preview** = the live gauge + the real-time fuel-zero gate trigger only. The gateway's per-request token estimate moves the gauge between anchor reads and is **never trusted over the provider's billed cost**.
