...
- **Fuel-stop mechanism (d69, 2026-07-17; supersedes the spend-limit-as-ceiling reading of d39-41):** verified provider facts forced the redesign — workspace spend/rate limits are Console-only (no write API) and the spend limit is a MONTHLY-RESET cap; API keys cannot be created by API (Console only) but CAN be flipped `active`/`inactive` via Admin API; usage report = 1m buckets, ~5min lag, 1/min polling; cost report = daily only. Design (codex-adversarially selected, S1-plus-amendments over gateway-gate S2 and WIF S3):
  - **Reserve pool:** monthly Console batch creates N workspace+key pairs; in the same visit each gets LOW per-workspace rate limits (velocity bound; e.g. 200k ITPM / 20k OTPM ⇒ ≈$1.50/min max burn at Opus prices) and a coarse monthly spend fuse (≈$500) as catastrophe blast-radius. Checkout reserves a pair (blocked if none — before payment); kickoff binds it.
  - **Stopping distance:** H_off = V×(L_max + P_poll + A_max) + margin, where V = $/min from the workspace's OWN rate limits, L_max = enforced usage-report freshness deadline, P_poll = 1min, A_max = confirmed key-flip time. Stop (key→inactive via Admin API) fires at remaining ≤ H_off; re-arm at H_on = H_off + max($5, 2×V×P_poll) (hysteresis). Unburned stopping distance is never lost (usable after top-up; released at settlement).
  - **Stop-cause precedence:** archived > chargeback > AUP > NGO-revoked > exhausted; top-up clears ONLY exhausted. Durable state machine, monotonic generations, idempotent read-after-write-confirmed provider flips, reconciled on restart (fail-closed when unknown).
  - **Watchdog:** gateway passively logs per-response usage (already captured for REQ-034) + provider rate-limit headers; stale usage report while gateway sees traffic, or unconfirmed flip ⇒ fail closed: cut project virtual keys + page admin. Credential-status action; request path never computes money.
  - **Month boundary:** cumulative fuel math never resets; only the fuse resets. **Settlement:** completion/cancellation = fence → archive → wait for final daily cost report → release remainder to general balance. **Rate card:** versioned; unknown model/billable component ⇒ fail closed. Live gauge = provisional (usage×official rates); ledger = billed cost (nightly conform).
  - Empirical TODO before build: measure key-flip propagation latency, inactive-key behavior on in-flight streams, real usage-report lag distribution (live probe on our org).
- **Claude Code auth mode = API consumption, never subscription (d63, founder-confirmed).** Volunteer Claude Code runs on `ANTHROPIC_BASE_URL` → gateway + the virtual key (Claude Code's documented LLM-gateway support; standard client, unmodified). The volunteer needs NO Anthropic account/subscription; all burn lands on the project workspace pay-as-you-go. Subscription (Pro/Max) is structurally incompatible: personal OAuth, no workspace scoping, no spend limit, bills the volunteer, invisible to the org Admin/Usage API — every d39–41 pillar requires API-key billing. **Identity split (privacy property):** Anthropic sees gateway-origin traffic per workspace key → provider logs know projects, never volunteers; the volunteer↔request mapping exists only in the gateway's metadata log. **ZDR fit:** Claude Code is ZDR-eligible exactly in this mode ("pay-as-you-go API keys from a Commercial organization") → the RM-60 ZDR ask covers the volunteer path; client-side Claude Code metrics logging is ZDR-exempt (volunteer-side; never collected, Out-of-scope #23); Covered Models (30d retention, no ZDR) re-checked at any model upgrade — Opus compatible today.

### REQ-023 — Triage gate (d74, 2026-07-18; supersedes the d29 automated-screener design and d73's screener mechanics)
- **v1 = founder-decided review of EVERY publish + a no-authority advisory pass.** Codex-challenged removal reasoning (verdict at loop/out/d74-breaker-verdict.json): full removal lost 7 checks with no automated home and produced weaker eval data; autonomous approval was untunable at launch (no labeled data) with all its risk in its only power. Advisory-only won: one structured LLM evaluation at publish → versioned per-check evidence on the queue item; NO recommendation (anchoring avoidance), NO state transitions, outage → reviewer proceeds unaided. Founder outcomes: approve→open / return→scoped / decline→cancelled (approve is new — was the screener's exit). Decision records carry per-check dispositions + policy version + advisory output/version + override rationale = RM-64's calibration dataset (advisory runs in shadow → measurable FP/FN before authority is ever granted). OD-8 retired → RM-64 activation gate. Promise §8 carries the explicit pilot exception (human-reviewed publications). v1 accepts a paused queue during founder absence (no backup reviewer — revisit at RM-64). `[superseded: confidence thresholds, uncertainty routing, auto-approval, Tier-2-never-auto, post-hoc spot-checks]`
...
- **Requirement-anchored NGO comment relay (Option B mechanics, d54; anchor unit re-ruled to the PM-tree requirement by d86 under d82's two-tree split).** The NGO comments on a requirement item from the read-only panel → the platform **posts it into that requirement's PM-tree Linear item as a comment** attributed "NGO (via ai4good)" — a scoped platform write extending the existing issue-create authority; it is not a status change, so detect-and-revert does not apply. The volunteer sees it inline on the requirement item (queued or in-progress) and is notified; the volunteer's deliberate reply routes back to the REQ-015 thread, while routine dev-internal Linear comments never surface to the NGO. Dev-tree items are never NGO-facing anchors. NGO never touches Linear. Pulls the requirement-anchored piece forward from RM-43 (the full channel stays deferred).
...
- **NGO-only bot interface:** the assistant is surfaced solely to the project's NGO account. The project page's PUBLIC STATUS PROJECTION stays role-uniform, while role-gated elements (the participants-only comment thread REQ-015, restricted file downloads REQ-032, and this bot) sit outside that equality rule per d80 — the bot is never shown to the volunteer, platform visitors, or the public (it spends the NGO's fuel and answers the NGO's "how is my project going?").

### REQ-034 — Attribution → see **Cross-cutting A**. **Mechanism in one line (d61, answering "how does attribution work / firing events sounds bad"):** self-assign → the Skill writes the task ID per worktree (the binding) → the binding rides each gateway request → the gateway, while proxying the request it already handles, INSERTs one row into the single append-only usage log (task or exploration/onboarding/unattributed floor); the platform backend does the same INSERT for its own Discovery/assistant calls. Two writers total, both already on the money path; no event bus, no subscribers, no fan-out; all views (burn-per-deliverable, buckets, reconciliation) are SQL reads over that one table, computed on view. **Why a log, not counters (d61-r, founder asked):** double-entry principle — the journal is the record, balances are derived. Counters break four promised properties: reconciliation can't localize drift vs provider truth (only blind overwrite — kills REQ-030's traceable corrections + refusal-to-guess); increments aren't idempotent under retries/aborts (rows keyed by request id are — and a safe counter needs a processed-ids dedupe record, i.e. a log anyway); REQ-006's "share locked per consumption, never retroactive" needs per-request `rate_card_version`; soft attribution (REQ-034) needs historical re-bucketing, impossible after a wrong increment. Asymmetry: counters are always derivable from the log (add a rollup view if reads ever slow); the reverse never. Write cost identical (1 INSERT vs 1 UPDATE; the INSERT avoids hot-row lock contention under parallel agent loops). Pilot volume ~hundreds of rows/day → SUM on view is instant. **Token-native attribution (d64; corrected d64-r, founder: "Money never derived!"):** the attribution meter is token-denominated END TO END — burn per task/bucket/category is measured AND displayed in tokens; there is NO dollar derivation from token math (the interim scale-to-billed allocation idea is dead). The fuel/money meter renders provider truth directly: the Anthropic Cost/Usage Admin API per workspace + Lovable `get_workspace` credit status. Reconciliation is same-unit only: row token totals ↔ the provider Usage API's token aggregates (tokens against tokens); the money ledger ↔ billed cost + Stripe (dollars against dollars). Rows carry NO money fields (no computed cost, no fuel delta, no rate-card version — the platform share is recognized ledger-side on billed cost, REQ-006). Consequences: no false dollar precision at task granularity, no rate-card coupling in attribution history, simpler rows; baselines in tokens survive price changes and compare across time. **Storage/leakage map (d61-r2, founder asked about prompt/response storage):** the usage log stores NO bodies — ~250B metadata/row (triple-stated invariant: REQ-034, REQ-009 privacy invariants, Out-of-scope #21 "permanently out"; #23 bans OTel content collection), so even year-1 agent-heavy volume is single-digit GB/year. Where content DOES exist: gateway in-flight only (transient inspection, no persistence — a gateway compromise reads live traffic, not history; there is no history); Anthropic (inherent; commercial API no-training default + bounded retention; Tier-2 real data never enters prompts — fixtures-only); the volunteer's own machine (never collected); and the ONE deliberate store — Discovery/assistant chats, the NGO's own resumable product conversation, tenant-isolated, with never-paste-sensitive-data warnings. No warehouse of prompts exists to breach. **Future forensics question (d62, founder 2026-07-14):** if the platform succeeds, inter-party disputes may motivate content-level evidence. On record: (1) metadata + public artifacts already cover the realistic allegations (misdirected fuel → buckets/tripwire/timing; bad code → the public repo/commits/Lovable+Linear history; value-for-money → burn-per-deliverable); bodies mainly add intent evidence. (2) RETROACTIVE content forensics is impossible without always-on storage of everyone's prompts — a direct amendment of Out-of-scope #21 ("permanently out, deliberate and on record"), a rewrite of the volunteer disclosure into content-capture consent, GDPR/minimization exposure, and the creation of the exact breach target the current posture avoids. (3) If ever adopted, the shape is **prospective incident-triggered preservation** (litigation hold): default never-store; on a formally opened dispute, preservation starts from that moment, scoped to that project, with notice to both parties, time-bounded, counsel-reviewed. (3b, d63-r) **The gateway is the only possible tap for volunteer build traffic** (provider gives no content back; the volunteer's machine is theirs) — and the capability is latent: bodies already transit the gateway, so persistence is one switch away; the policy invariant ("never bodies", Out-of-scope #21) is the real control, which is why it is stated as an on-record commitment. If ever flipped: captured content goes to a **separate encrypted evidence store — never the usage log** (money/telemetry and content evidence must not share a table, access path, or retention policy). The NGO-facing surfaces need no gateway tap (Discovery/assistant content is already product data); async platform workloads can use Batch for a provider-side 29-day record. Not an RM item — amending the permanently-out list is a founder-level posture decision, deliberately left outside the roadmap. **Provider-side retention verified (d62-r, corrected d62-r2; platform.claude.com 2026-07-14):** per surface — **interactive Messages API (the volunteer path): no supported customer content read-back**; internal retention only (default: content NOT at rest post-response; Covered Models Fable-5/Mythos-5 REQUIRE 30d; 30d togglable per workspace; ZDR per org via sales; T&S-flagged up to 2y). Surfaces that DO log readably: **Console request logs** (metadata only; the /api/logs endpoint is undocumented + WAF-fronted — not buildable-on); **Batch API** (FULL request/response content downloadable for 29 days — the one provider-side content log; async-only, not ZDR-eligible; gateway policy keeps batch per-project opt-in RM-47); Files API (until deleted); Managed Agents sessions (until deleted; different product); client-side Claude Code OTel (prompt content only if the volunteer enables it, to their own backend; ai4good never collects, Out-of-scope #23). → Dispute forensics on interactive build traffic is gateway-side by necessity; batch-routing is a legitimate tool for async platform workloads (e.g. scorer/screener runs) if a 29-day provider-side record is ever wanted — trading away ZDR for that traffic. OPPORTUNITY: bundle a **ZDR request into the Anthropic commercial ask (RM-60)** — extends "metadata only, never bodies" to the provider tier (Tier-2 story: volunteer prompts not stored at rest anywhere). CAVEAT: ZDR excludes Covered Models (30-day retention mandatory) — Opus is compatible today; re-check at any model upgrade; Anthropic's retention toggle is per-workspace, mapping 1:1 onto per-project workspaces (d39), so a mixed posture is expressible per project. Additional: **attribution spans both billable surfaces (d55)** — gateway (volunteer build → task or taskless bucket) and **direct** (NGO-facing platform AI → project categories: funded Discovery, the NGO assistant), so direct-surface tokens are counted + attributed to the project, never uncounted or lumped into build burn; the audit event's surface/actor (REQ-029) drive the category. Aggregation boundary — NGO sees burn per deliverable + per NGO-facing category **in tokens (d64-r), no celebration**, with exploration/onboarding/unattributed as honest buckets reconciling to total token usage; per-volunteer-per-task granularity + per-project unattributed-% / binding-failure signals stay coordinator-side; wide per-task burn variation treated as a data property, not an anomaly.
