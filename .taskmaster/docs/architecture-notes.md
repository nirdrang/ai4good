...
- **Role model (verified against Lovable's full workspace role matrix, 2026-07-13).** From the matrix: *Create new projects in a workspace* = Editor **Yes**; *Manage Supabase* (enable/manage the DB) = Editor **Yes**; *Manage GitHub* = Editor Yes; *Publish* = Editor Yes; *Manage plans & billing* = Editor **No**; *Manage Integrations* (workspace-level connectors) = Editor **No**; *View usage* = Editor **No**. Only *Disconnect Supabase* (destructive, project-level) is admin/owner. So the volunteer is invited as **Editor** and does the **entire technical setup**; the NGO's role collapses to account/billing. Kickoff split:
...
- **Credit-display read mechanism — server-side bot (INTENDED, conditional on the `[verify]` below; founder 2026-07-13, d50).** Preferred over the volunteer's in-session read, which is fragile and dev-machine-dependent. Design: a dedicated **ai4good bot Lovable account** is an **editor** member of each NGO workspace; a **server runs headless Claude Code** with the Lovable MCP authed once (browser) as the bot; a **skill loops `get_workspace` over the active-project `workspace_id` list**, computes OK/low/exhausted, and POSTs to an ai4good edge function → per-project cache → UI (REQ-010). Why it's buildable despite Lovable exposing no server/machine auth (OAuth-only, whitelisted clients, no API keys — verified): **Claude Code is a whitelisted OAuth client**, so headless Claude Code is a *supported* client, not a custom server (which Lovable blocks). Editor role keeps billing locked. **Invite chain (B):** the NGO invites only the **volunteer**; the volunteer (an editor can *Add Editor*) invites the **bot** — the NGO stays at one invite; offboarding removes both (an editor can *Remove Editor*).
  ...
  - **Pending pure-doc edits when adopted (post-proof), NOT yet applied:** REQ-021's "credit status read via the volunteer's connected Lovable session" → mechanism-neutral (read is platform-side); REQ-021's "ai4good is never a member of the NGO's Lovable workspace" → "ai4good's only presence is a read-only editor bot for credit monitoring, removed at completion"; offboarding removes both the bot and the volunteer. Add a roadmap item then (referenced from the pure doc to keep the RM bijection) for the **durable Lovable service-account / API-key** integration — the headless-Claude-Code setup is the interim; a real API key is the durable replacement. Raise that API-key ask with Lovable in parallel (their docs invite "contact support for alternative authentication").
