...
### RM-64: Autonomous triage screener (decision authority)
**Source:** REQ-023 (v1 = founder-decided triage with a no-authority AI advisory pass, d74; supersedes the earlier automated-screener design)
**Trigger/Gate:** sustained publish volume breaching the founder review target, AND calibration evidence from the accumulated v1 review records — per-check false-negative targets met, a bias review of founder-decision labels, defined rollback behavior, and reconciliation with RM-53 (self-attestation spot-checks) and RM-57 (KYC auto-approval) so bypass strategies do not multiply.
v1 deliberately reviews every publish by hand: the advisory pass runs in shadow (per-check evidence, no authority), so the review records (per-check dispositions, policy version, advisory output/version, override rationale) form a measurable false-positive/false-negative dataset BEFORE any authority is granted. This item restores decision authority: confident-clean auto-approval to `open`, an uncertainty threshold + model configuration (the questions formerly parked as OD-8), Tier-2 always-human routing, and post-hoc spot-checking of auto-approvals.
