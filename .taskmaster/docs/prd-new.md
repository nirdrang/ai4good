...
- Volunteer marks "Ready for Handoff" once all must-have PM tasks are done (open GitHub Issues never block — dev-internal); an automated checklist confirms completion quality — README, runbook, deploy instructions, at least one passing CI run, license file (MIT default) — not a delivery gate, since the app is already live and the NGO's.
...
A lightweight workflow from "code done" to "NGO operating solo." Because Lovable is enforced and git-bound (decision-23), the running app + repo are already the NGO's throughout — so handoff delivers/transfers nothing; its real substance is **the access transition (offboard the volunteer) + the bookkeeping** (attribution, credit, fuel release, `handed_off`). Founder review 2026-07-09.
...
- **Repo handoff (uniform):** the repo stays in the platform org with the NGO admin promoted to admin; the volunteer is removed or dropped to read-only **at the NGO's choice** — no transfer step (redundant: MIT + Lovable two-way sync already make the code the NGO's to fork or export).
