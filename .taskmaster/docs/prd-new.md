...
- `matched_pending_fuel` → `in_progress` on funding (≥ $50): the kickoff sequence fires. After 7 days without funding, the project returns to `open`, the volunteer is freed and notified, and the NGO gets a re-fund prompt. The NGO can also cancel the match anytime pre-payment.
