...
- At first project application (the first-commitment moment, REQ-007), the volunteer must accept the ToS + Platform Promise clauses: coordination layer only — no obligated outcomes; volunteering, not employment; all work public open-source by default (MIT; private only with NGO justification); each project's API key used only for that project's work; fixtures-only plus confidentiality on Tier-2 data; violations risk account deactivation and forfeiture of completion credit. Acceptance is per account; a material text change forces a one-time re-accept.
- Marketplace shows scope, suggested stack, cause tags, and the volunteer's own skills as plain tags for self-assessment — no computed overlap badges and no numeric match score in v1 (REQ-011; overlap computation joins the v1.5 match score — decision-26). Volunteer applies; NGO approves/declines or auto-approves (configurable). Acceptance does NOT trigger kickoff — the project waits until the NGO funds it (match-to-fund, REQ-006).
...
**Chargebacks + reserve:** on a payment dispute the platform immediately freezes the NGO (no new funding/matching), cuts the project's AI access, claws back the unconsumed balance, books the consumed portion as a platform loss, and opens an admin review; the audit-logged acknowledgment (timestamp + IP) is submitted as dispute evidence — what wins friendly-fraud disputes. Loss stays bounded by no-cash-out + the first-fund cap + rapid access cutoff. A small chargeback reserve (~$5–10k year-1, resized on observed loss rates) is maintained as documented treasury policy, visible on the daily money dashboard — the automated coverage-floor alert is v1.5 (decision-26). Fraud posture (decision-26 + founder verdict): collusion/shared-fingerprint detection is removed from v1 — concierge hand-vetting of every pairing + no-cash-out + caps are the v1 control; the full anti-Sybil program is v1.5.
...
- Cards show the project's needed skills and cause tags plainly (the volunteer reads their own fit); computed match badges join the v1.5 match score (decision-26).
...
Kill switches, admin enforcement, and ops queues previously routed to one admin with no on-call model, restore runbook, false-positive reversal, or money-correction tooling. This REQ names the operating model.
...
- Every automated or admin enforcement action (key revocation, account deactivation, status revert) is reversible by an admin, audited. **Money corrections (v1 — decision-26): no correction UI; one privileged, audited database function is the only correction path** — direct writes to ledger/correction tables are revoked from all app/admin roles; the function requires actor + reason + idempotency key (no double-apply), enforces pair-sum zero, and validates the referenced external object (payment/NGO/project mapping, amount, currency) against synced Stripe records before applying (codex-required). The correction UI is v1.5.
...
- Matching: application received → NGO (in-app, per-event — digests are v1.5); application accepted / declined → volunteer (email + in-app; a duplicate NGO-side "matched" event was collapsed); unmatched open project aging → platform admin + matching volunteers (concierge hand-matching + supply-liquidity nudge, Goal 5).
