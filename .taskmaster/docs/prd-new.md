...
> Distilled product-requirements doc. Implementation and architecture detail lives in the design docs and the Linear task tree.
...
**Legal posture (decided 2026-06-03; counsel review postponed post-MVP — founder verdict, flow #17 2026-07-08):** a plain-language ToS carries the protective terms (no warranty, no SLA, limitation of liability), because such terms are only enforceable inside a contract. The MVP pilot runs on the un-reviewed plain-language draft — risk accepted. Counsel review, the volunteer-classification opinion, and the fiscal-sponsor agreement are post-MVP items. ai4good asserts a limited, bona-fide coordination relationship — not "no contract exists." Volunteers donate time to charitable work; they are not employees, contractors, or subcontractors of ai4good or the NGO.
...
- Transparent volunteer track record — completion credit is recorded from day one (public display and badges arrive in v1.5 — decision-29/r2; every repo is public MIT, so the working portfolio exists on GitHub from the first commit). Never stars.
...
- Escalation paths when work stalls — messaging (REQ-015) and notifications (REQ-016). Post-handoff issue surfacing (REQ-017) is v2; in v1 NGOs re-engage by posting a new project.
...
A two-sided marketplace dies on the side with no liquidity — for ai4good, that side is supply. Launch is concierge-first: pre-recruit a volunteer bench and hand-match the first ~10-15 curated projects before opening organic browse (REQ-007). Metrics: the volunteer activation funnel (signup → first match consent → first handoff; "first apply" joins the funnel with organic browse in v1.5) and time-to-first-match per open project — both fed by the concierge match log (REQ-007). Target: a pre-launch bench of ≥20 volunteers; ≥80% of the first cohort's open projects matched within 7 days; signup→handoff activation ≥25%. A project unmatched past 7 days surfaces to the concierge queue for hand-matching (REQ-016), at most once per project per 72h (volunteer-facing featuring arrives with organic browse).
...
- The project page shows dual meters: the Claude Code fuel gauge and Lovable status (REQ-010). Abuse protections beyond virtual keys, caps, inline metering, and the fuel gate are documented-not-built in v1 (REQ-009).
...
- No GitHub Issues are auto-created from scope; they remain dev-internal and never appear in the NGO view (REQ-008/REQ-026). Volunteer-added sub-issues join the tree; an accepted scope addition is a volunteer-created linked task (REQ-025 v1 protocol — the "Change Request: [title]" parent convention returns in v1.5).
...
NGOs sign up (email-verified) and complete an org profile. In v1, trust is the founder's hand, recorded properly. The self-serve verification pipeline — doc upload UI, pending-review queue, document viewer, public badge — is deferred to v1.5, because vetting already happens in concierge onboarding: the founder talks to every pilot NGO before admitting it. Two trust tiers in v1; the KYC tier stays v1.5.
...
- **v1.5 gate (explicit):** self-serve doc upload, the review queue + viewer, badge display, and the KYC tier all return before ANY non-concierge admission path opens (organic NGO signup, EU/UK signup). A paid "Discovery wallet" stays excluded in v1 AND v1.5 (decision-8).
...
Match records track their own states — invited / consented / declined / expired / released — in the concierge match log (REQ-007). A match that expires unfunded frees the volunteer for a later re-match. (Self-serve application states arrive with organic browse, v1.5.)
...
The internal arithmetic invariants (control totals) verify nightly and auto-repair by the same provider-truth rules. Full double-entry accounting is a v1.5 upgrade, once an accountant defines the chart of accounts. Match expiry (7 days) is enforced automatically.
...
**Chargebacks:** on a payment dispute, the platform immediately freezes the NGO (no new funding or matching), cuts the project's AI access, claws back the unconsumed balance, books the consumed portion as a platform loss, and opens an admin review. The audit-logged acknowledgment (timestamp + IP) is submitted as dispute evidence — that is what wins friendly-fraud disputes. Loss stays bounded by no-cash-out + the first-fund cap + rapid access cutoff. A formal chargeback reserve is deferred to later stages entirely (founder verdict, flow #11 2026-07-08 — not an MVP item); at pilot scale, chargeback losses are absorbed from operating funds as they occur, bounded by the same three controls. Fraud posture (decision-26 + founder verdict): collusion/shared-fingerprint detection is removed from v1 — concierge hand-vetting of every pairing + no-cash-out + caps are the v1 control; the full anti-Sybil program is v1.5.
...
- **Org removal by cause:** (a) voluntary deactivation — membership removed, but per-project access on active projects stays so in-flight work can finish; (b) **AUP enforcement (v1 minimal — founder verdict 2026-07-08):** the admin deactivates the account (the lifecycle state gates every platform write) and instantly revokes the project's virtual keys, with an audited note. Remaining access — repo, Linear membership, org membership — is removed via a short manual checklist, and the platform removes the volunteer from the Lovable workspace via its build-phase member seat (decision-35). Reversal = the admin re-enables the account and re-issues keys, manually. The orchestrated staged-teardown saga and lazy per-project rebuild machinery are v1.5; (c) 24-month inactivity — soft removal, like (a).
- The dashboard shows GitHub status ("Linked as @handle" / "Not linked — link now to accept a match").
- Public project listing (read-only in v1): title, summary, complexity tier, suggested stack, cause tags, NGO name, posted date — newest-first, with no filter/sort machinery (v1.5 with the browse surface; no public verification badge in v1, decision-29/r3).
- Deferred to v1.5 with organic browse: filters, overlap badges and the match score (decision-26), Apply + cover note, the NGO applicant-review queue, and auto-approve settings.
...
- **Key-leak hygiene (v1 = prevention, not detection — decision-26):** env files are ignored by default, and the platform key pattern is registered with GitHub secret scanning + push protection. The key-leak canary and the volunteer self-audit usage view are deferred to v1.5 (gate: organic volunteer signup) — at hand-vetted pilot scale, caps + project binding + the fuel gate already bound exposure. Org-namespace guard (near-zero build; a decision-26 guard): org settings restrict members to private-only repo creation, visibility changes to owners/platform automation, and Pages/Actions for unmatched repos — all asserted by the same continuously-checked base-permission invariant (REQ-008). Project repos flip public only after the setup checklist validates the repo URL, and the founder's daily review includes a one-call org-repo-vs-active-projects comparison.
- **Escalation ladder documented, NOT built in v1** (listed so the decision not to surveil stays visible): activity-vs-burn reconciliation on shape signals only, phase-aware (trigger: real ledger anomalies; **OD-5**); marker rotation with a mandatory grace window (trigger: adverse offboarding or a leak); a short-lived per-volunteer signed token checked pairwise against the key (trigger: repeated anomalies that conversations don't resolve); sampled content classification (trigger: probably never).
...
- v1.5 (deferred, with organic browse): filters, sort options, the apply flow, a deterministic match score with a visible breakdown, and NGO satisfaction as a private weighting component.
...
- On accept: project → `handed_off`; leftover fuel → the NGO general balance as non-cash credit (REQ-006 — not a cash refund); the volunteer's completion-credit event is recorded with a private confirmation (public display + badge v1.5 — decision-29/r2); all project virtual keys terminate (REQ-009); Linear membership is removed and the task tree snapshotted (REQ-026); the **REQ-035 attribution step** is captured at sign-off.
...
Lovable is the primary deliverable vehicle and the NGO's durable maintenance home: after handoff, the non-technical NGO evolves the live app via Lovable's chat — no developer needed. During the build, Claude Code is the volunteer's single entry point: it orchestrates Lovable for the UI layer and handles backend, logic, tests, and docs directly, keeping ai4good's metering, scope enforcement, and audit authoritative over Lovable work. Every project uses Lovable (single delivery model — decision-23); the skip-Lovable opt-out is removed (reserved post-v1).
...
4. Credit status is manual in v1. The volunteer sets a Lovable status widget (active / credits low / blocked); low or blocked shows the NGO a "Top up Lovable credits" CTA deep-linked to Lovable billing, and the NGO clicks "I've topped up — unblock" to reset. Automatic detection by parsing Lovable's notification emails is deferred to v1.5: it is brittle, needs inbound-mail infrastructure, adds NGO setup friction, and the widget already yields the same product behavior.
...
**Deferred beyond the v1.5 spec:** a dedicated CR section grouped by status; automatic completion detection + event; an AI agent drafting acceptance criteria from CR text; NGO cancellation of a pending CR. v2 — mid-project scope reduction (informal today); project splitting (not planned).
...
- Session bootstrap primes each session with the scope summary, current status, in-progress tasks, unresolved blockers, recent NGO comments, and fuel runway; verifies task-system access; reads the task binding; and prints a one-line banner (project, active task + age, fuel remaining, unread NGO comments). (The CR inbox returns in v1.5 with the structured CR surface — decision-29/r1.)
...
- v1 dashboards = the ONE money dashboard (funding, consumption, platform share, reconciliation, chargebacks) that the founder reads daily. The broader golden-signal dashboard suite is v1.5 (decision-26). Error tracking and structured logging stay baseline NFRs.
...
- Account deactivation (v1 AUP enforcement, founder verdict) has a documented recovery playbook: re-enable and re-issue keys, manually. The suspension state machine is v1.5.
...
- v1.5 (gate: before organic/EU signup): a universal "Report" affordance (repos / Discovery output / messages) feeding an abuse-review queue; a rapid, reversible takedown state (repo private + "under review", decoupled from suspension); DMCA agent registration + a documented CSAM→NCMEC escalation; AUP suspension optionally flips the project's repos private.
...
- The volunteer downloads files from the project page as build reference (the Skill's local sync command is v1.5 — decision-26). The repo template gitignores the standard download path, and files are never committed to the repo — keeping Tier-2/sensitive content out of git history. (The residual accidental-commit risk at pilot scale is accepted, mitigated by secret scanning and the Tier-2 fixtures-only rule.)
...
- Reuses the Discovery surface and model — no new chat infrastructure. v1 is on-demand text Q&A only; proactive/push digests and scheduled summaries are v1.5. (No scope guardrail here — the assistant is funded-only, and paid usage is the NGO's own call per decision-34; the per-turn cost display is the control.)
...
- v1: capture + the NGO burn-per-deliverable view. v1.5: coordinator reconciliation surfaces (per-project unattributed %, broken/stale-binding signals) plus per-task cost baselines feeding Discovery estimates.
...
This closes the freedom-without-guarantee model economically: no gates anywhere — quality becomes visible after the fact, and reputation is the incentive. Capture ships in v1 (uncaptured data is lost forever); synthesis and matching surfaces land in v1.5.
...
2. Post-handoff health (v1 = reachability + a structured human check-in; 60/90-day automation is v1.5): the 30-day automated ping stays — reachability-only ("deployed and responding," not evidence of self-maintenance). The longitudinal layer is replaced in v1 by a founder check-in at day 45–60, run as a REQUIRED ops item. The check-in task is created at handoff acceptance (handoff cannot complete without it — owner + due date). The outreach is closed-form and PII-minimizing: six fields only (self-service attempted, worked/failed, URL reachable, failure category, follow-up owner, follow-up due), and the message itself instructs never to send screenshots, beneficiary data, secrets, or raw incident detail — privacy/security concerns route to the incident path before details are requested. Any failure gets a follow-up owner with a 2-business-day clock. v1.5 automates the 60/90-day checks plus real-signal synthesis, designed from what the manual check-ins actually reveal. Health signals never notify the volunteer punitively.
3. Reputation feeds matching (v1.5): the attribution + health record becomes visible during matching. No gates — a volunteer with weak signals still applies; the NGO sees the record.
4. AI-maintainability check (v1.5, signal only — never a gate): an optional visible test at handoff — a fresh agent runs realistic change requests against the repo; the result is shown, and nothing is blocked.

### P0 (promoted from P1): Required dependencies of REQ-024 / REQ-025 / REQ-026
REQ-013/014/015/016 were drafted P1 but are dependencies of P0 features (blockers, scope additions, PM tasks) — so they are reclassified P0, each shipping a minimal v1 cut with enhancements in v1.5. REQ-017 moves to v2; no P1 work in v1.
...
- v1: project cards (status, % complete from tasks, dual fuel meters, assigned volunteer) with cadence signals per card — last commit, tasks X of Y done, "Now working on: [task]" (the Active/Quiet/Stalled badge is v1.5 with the cadence extras — decision-26/c3). A fuel summary across projects, with the general balance shown as "$X redeployable credit" (non-cash; no expiry, never removed). An "Action needed" rail: open blockers (fuel top-up, Lovable credits, collaborator needed, clarifying questions), open scope-addition discussions, and triage decisions awaiting the NGO. (No applicant queue in v1 — matching is concierge, decision-28.)
- v1.5: a 30-day activity feed; a KYC upsell banner for verified NGOs; a Lovable-enabled-projects rail; opt-in NGO testimonial authoring.
...
- **Public profile + badge deferred to v1.5 (decision-29/r2):** no public profile page or badge display in v1 — their audiences (organic NGOs, algorithmic matching) arrive with the v1.5 marketplace. Completion credit is captured from day one as **append-only per-project events** (volunteer, project, handoff-accepted timestamp, first-tool eligibility), so v1.5 reconstructs the full display without archaeology. At handoff the volunteer gets a private "credit earned" confirmation. Copy is honest: "credit recorded from day one; public reputation display arrives in v1.5" — and every repo is public MIT, so the actual portfolio already exists on GitHub.
- NGO-satisfaction signal collection is deferred to v1.5 (it arrives with the tips/satisfaction handoff UX): no satisfaction modal at handoff and no admin-visible aggregate in v1; the volunteer never sees their own satisfaction scores (this avoids anxiety and gaming).
- v1.5: a multi-badge engine (5/10 tools shipped, active contributor, cause specialist); a handoff history view; opt-in NGO-authored testimonials; cause-specialization tags.
...
- v1: a chronological free-text stream; plain text with auto-linked URLs (no markdown, code blocks, attachments, or @-mentions). It loads on page view — no live push; posting notifies the other party. Membership is implicit from project roles. System events (funding, setup, fuel-low, handoff, blockers) never post to the thread — they surface in the notifications and project activity feeds. (Scope-addition discussions live here in v1 per REQ-025; the actionable CR row returns in v1.5.) After handoff the thread is read-only. No cross-project DMs.
- v1.5: a real-time channel with threaded replies; @-mentions, presence, typing indicators; markdown, code blocks, image attachments; full-text search. v2+: a bridge to a real Slack workspace; video/audio calls.

#### REQ-016: Notifications (Email + In-App)
Event-driven email + in-app notifications, with documented defaults in v1; the per-user preference UI is v1.5. One shared emitter, driven by a single static event taxonomy, is the sole writer of notifications — blockers, scope additions, and lifecycle events all notify through it rather than sending comms directly.
...
- Scope additions (v1 informal — decision-29/r1): discussion rides project-thread comment notifications; there are no dedicated CR events in v1 (the structured CR taxonomy returns in v1.5 with the surface).
...
Teams of 2–3 volunteers per project, with role splits (frontend/backend/QA). (v1 is strictly single-dev — decision-28.)
...
- REQ-002 minimal — NGO trust via the audited founder-vetted action during concierge onboarding (decision-29/r3); the self-serve verification pipeline, KYC tier, and automated identity verification are v1.5.
...
- REQ-011 minimal — Public read-only, newest-first project listings; the apply flow, filters, badges, and match score arrive with organic browse in v1.5 (decisions 26/28).
...
**Funding & Money:**
- REQ-006 — Stripe top-up + fuel ledger + match-to-fund under the non-cash, no-cash-out model. Leftover releases to the general balance and auto-applies at future checkouts (no donation flow — decision-28); chargeback handling and first-fund caps included (the formal reserve is deferred — flow #11). No payout, cash-refund, payee-KYC, or AML rails in v1; no refunds of any kind (flow #9). Auto-top-up v1.5.
...
- REQ-025 minimal — Mid-build scope additions via the informal comment-thread protocol (decision-29/r1): volunteer accepts in-thread and creates the linked task before any spend; the structured CR surface is v1.5.
...
- REQ-014 minimal — Volunteer dashboard: current projects + fuel gauge + completion-credit events (recorded from day one); the public profile and badge display are v1.5 (decision-29/r2).
...
2. Refund/donation/chargeback mechanics — dissolved: no refunds of any kind in v1 (flow #9), the leftover-fuel donation flow is removed (decision-28), and chargeback-after-consumption is handled (freeze, access-off, write-off; the formal reserve is deferred, flow #11).
...
9. Counsel deliverables — POSTPONED post-MVP (founder verdict, flow #17 2026-07-08; the pilot runs on the plain-language draft ToS, risk accepted): counsel-reviewed Terms of Service (no-warranty, no-SLA, limitation of liability; the UI keeps the warm voice); volunteer-classification opinion; fiscal-sponsor agreement; EU data-residency + sub-processor/DPA stance before EU onboarding.
