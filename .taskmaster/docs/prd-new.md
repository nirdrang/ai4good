...
- **At handoff:** the volunteer is downgraded to read/triage and the NGO admin promoted to admin — no org transfer (the repo's permanent home is the platform org). The NGO removes the volunteer from the Lovable workspace separately (only the workspace owner can).
...
5. Handoff: the NGO removes the volunteer from the Lovable workspace and keeps the workspace + billing relationship. The repo stays in the platform org with NGO admin access (no transfer step — decision-24/33); Lovable's GitHub sync continues without reconfiguration.
