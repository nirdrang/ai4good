...
- **Scope guardrails (decision-34, founder 2026-07-09) — no drip of unrelated execution:** Discovery is for scoping THIS project need, and the guardrail applies on both free credits and funded fuel (the paid expedite path is where drift costs the NGO real money). Three layers, matching the platform placement doctrine: (1) the platform-side system prompt carries a scope line — decline unrelated tasks (general Q&A, document drafting, translations, coding help) and redirect to scoping, mirroring the gateway's governance line for volunteers; (2) a deterministic per-conversation turn ceiling (platform-configurable, pilot-tuned) — past it, the agent moves to wrap-up and the composer requires generating the scope or starting a fresh Discovery, bounding any drip regardless of topic; (3) repeated off-topic declines in one conversation surface a plain notice to the NGO ("Discovery scopes this project; it can't help with other tasks") and flag the conversation for founder visibility in the daily review — visibility, never a lockout.
...
- The same scope guardrails as Discovery apply (decision-34): the assistant answers about THIS project only, declines unrelated tasks with a redirect, and carries the same turn ceiling and off-topic visibility flag — it is fuel-metered chat, so drift here costs the NGO the same real money.
...
- **34 — Discovery scope guardrails (2026-07-09, founder: "guardrails against a drip of unrelated execution").** Discovery (and the REQ-033 assistant) scope-guard their conversations on both free credits and funded fuel: a platform-side scope line in the system prompt declines and redirects unrelated tasks (mirroring the gateway governance line volunteers get); a deterministic per-conversation turn ceiling (pilot-tuned) bounds any drip regardless of topic; repeated off-topic declines show the NGO a plain notice and flag the conversation for founder visibility — visibility, never a lockout.
