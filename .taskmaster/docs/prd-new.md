...
- On funding, kickoff fires with no admin ops tasks: NGO + volunteer land the repo via the Lovable setup checklist (Lovable is mandatory — decision-23, REQ-021); per-project AI keys are issued (REQ-009); the project comment thread opens (REQ-015). The task backlog is NOT instant: a draft tree is decomposed from Discovery output and goes live in the project's Linear workspace only after the coordinator's decomposition review — the one human gate in kickoff (REQ-026; reviewer identity = OD-1; GitHub Issues stay dev-internal, REQ-008). Repo setup and key issuance never wait on it. Volunteer receives a starter kit: build guide + pull-based task workflow.
...
- **Repo handoff (uniform):** the repo stays in the platform org with the NGO admin promoted to admin and the volunteer downgraded — no transfer step (redundant: MIT + Lovable two-way sync already make the code the NGO's to fork or export). Private/Tier-2 projects stay private within the shared org in v1 (a separate private org is v1.5 hardening).
