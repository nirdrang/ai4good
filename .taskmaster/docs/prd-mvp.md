...
- The volunteer browses public listings and marks candidacies; the first candidacy starts the project's time-based assimilation window, and at lapse the concierge matches from the pool (decision-36).
...
- **36 (2026-07-11)** — Volunteer candidacy precedes matching (amends 28): volunteers mark interest in-product on public projects; the assimilation window is time-based, starting at the FIRST candidacy (pilot-tuned; later candidates join the pool, never reset the clock; candidate-less projects just stay open under Goal-5 aging); at lapse the concierge enforce-match draws from the pool. Candidacies feed the admin match log only — no NGO-facing queue. GitHub link mandatory at volunteer signup (was at-first-consent); platform-org membership still waits for first match consent.
