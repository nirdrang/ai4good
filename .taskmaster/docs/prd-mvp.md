...
- **Every metered AI request — gateway or direct — emits one privacy-preserving audit event** (surface/actor, project, request id, provider status and cost, fuel delta, rate-card version, ledger linkage; metadata only, never bodies) — the data spine for attribution and reconciliation.
