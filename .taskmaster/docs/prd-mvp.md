...
Deployed tools NGOs use, end-to-end (post → discovery → match → build → deploy → completion), not just published repos. Metric: projects reaching completion — all P0 requirements done (REQ-012). Baseline 0. Target: 25 NGOs with working deployed tools in the first 12 months.
...
- On funding, kickoff fires with no admin ops tasks: the repo is established by the NGO and volunteer via the Lovable setup (Lovable mandatory, REQ-021); per-project AI keys are issued (REQ-009); and the comment thread opens (REQ-015). There is no dev backlog at kickoff — the PM tree is seeded (the scope requirement items plus the bootstrap item, REQ-026) and the volunteer pulls the bootstrap and authors the project PRD from the Discovery scope (REQ-036), raising NGO clarifying questions; an automated scorer measures PRD completion against Discovery. After a passing check, the AI recommends Close PRD to the developer. Developer-started PRD materialization then creates the planning package and dev-tree decomposition (REQ-026; GitHub Issues stay dev-internal, REQ-008). Repo setup and key issuance never wait on it. The starter kit is a build guide plus the pull-based workflow.
...
A plain-language hierarchical view; status comes only from deterministic events — the pull and verified completion on the PM tree, vendor merge events on the dev tree — never agent self-report; pull-then-complete at the requirement level (pull a PM item before working on it) is **enforced by the Skill (REQ-028)**, so every metered request carries requirement-level attribution context. (Depends: REQ-008, REQ-026, REQ-028.)
- The tree is materialized from the frozen PRD after a passing completion check and developer close (REQ-036): one parent per story, one sub-issue per acceptance criterion; Discovery is the PRD's source, never decomposed directly; automation drafts and pushes and the coordinator spot-checks during the pilot.
- The ai4good project page is the primary view (current work, task list, activity); NGOs never need Linear seats.
- The pull marks a PM requirement in progress; verified completion is the only path to PM done, while dev-tree items flip on PR merge vendor-natively (revert-exempt, never NGO-facing); agents may comment and pull but never move PM status by hand (violations are detected and reverted); percent complete is done over total must-have PM items.
...
- Profile: skills, causes (optional, chosen from the system's existing emergent set — never typed or invented), availability (hours/week), optional bio.
...
- **GitHub Issues are dev-internal only** (bugs, refactors, tech debt), never NGO-visible; the Linear requirement (PM) tree is the source of truth for NGO-visible deliverables. No issues are auto-created from the scope doc.
- **Repo seeding:** each repo is seeded with the platform's session guidance and working conventions — the project-binding marker (REQ-009), requirement-binding conventions (REQ-034), the Linear working norms (one issue in progress, assign before starting, comment when blocked, never move status by hand), commit conventions that link work to its task, a reviewer-agent skill, and env-file hygiene. Platform API credentials are never committed; they are shown once and are separately revocable from the virtual key.
...
**PRD boundary:** the platform-owned PRD workspace calls OpenRouter through server-held credentials (REQ-036). Its billing handler reserves project funds and settles reported generation cost under REQ-006. Enforce project binding, membership, active-human authority, funding, and revocation before dispatch. Scoring and materialization require their own authorized operation. These rules do not change the Claude Code build gateway or make attribution rows a money source. Store the shared conversation as product authoring data, separate from the gateway telemetry log.
...
**UI work from Design sign-off:** PRD materialization also identifies the screens for Design review (REQ-036). A developer's sign-off creates or updates one UI development item for that screen. The item links to its PM requirement and the exact accepted screen revision, with the review decisions and sign-off actor and time. Retries and later sign-off update the same item. Reopening a screen retains the item's last accepted revision until the next sign-off. Updating the revision preserves existing work state and history. A signed-off screen becomes available in Build subject to its own dependencies, without waiting for other screens. There is no overall Ready to build gate. Screen sign-off does not complete development work or change PM requirement status. The NGO sees Design discussions and review status, never the generated development board.

**Provisioning:** each project is assigned its Linear workspace at kickoff with no manual step on the kickoff critical path — the volunteer is invited and the PRD bootstrap item is seeded (REQ-036). Provisioning that cannot complete surfaces as an external-dependency blocker rather than stalling silently or inventing a sub-state. The pilot runs on Linear's free tier, risk accepted (→ RM-30).

**Decomposition (from the closed PRD):** kickoff seeds the PM scope requirements and bootstrap, with an empty development tree. After a passing coverage check and developer close, developer-started PRD materialization creates the plan from the frozen PRD (REQ-036). Update PM requirements in place using stable requirement identifiers. Create or update one development parent per story and one sub-issue per acceptance criterion, with PM requirement links, priorities, and blocking relations. Retain session-sized briefs and coverage manifests. Reuse stable project, revision, requirement, and work identifiers on retry. UI work that needs a screen keeps its Design dependency; sign-off updates that planned item instead of duplicating it. The coordinator retains the pilot spot-check. Package completion supplies bootstrap evidence but does not complete product requirements.
...
- [ ] Screen sign-off creates or updates one UI development item with its requirement, accepted revision, review decisions, actor, and time. Repeated requests do not create duplicates.
- [ ] Build can use one signed-off screen while other screens remain in review or wait for NGO input. Reopening and re-signing preserve the item's identity, work state, and history.
...
- Helper commands: pick the next requirement (highest-priority unblocked PM-tree item, full context shown, self-assigns on confirm); fuel status (balance, burn rate, projected runway); list blockers (with suggested actions); raise a requirement-anchored clarifying question; a completion-readiness check (top-priority done, README + runbook, repo and deployment URLs set, work pushed); and disable/enable. Reference files download from the project page. (→ RM-31)
- Branch/commit conventions (dev-item identifiers + linking keywords) are auto-applied so the GitHub integration links and closes DEV leaves on merge (vendor-native); conventions never touch PM status — the pull and the complete flow are its only authorities (REQ-026). The volunteer can override anything the Skill generates. A manual fallback always exists — the Linear app and project page cover every behavior, and a disabled Skill still operates, with attribution degrading to unattributed and norms arriving via the injected prompt.
...
**Acceptance criteria:** install works end-to-end; bootstrap auto-runs with the banner; binding follows the PM-requirement pull per worktree; auth failure degrades without blocking; helper commands work; conventions apply with override (dev leaves only, never PM status); disable/enable persists; and docs cover install, configuration, troubleshooting, opt-out, and the disabling implication.
