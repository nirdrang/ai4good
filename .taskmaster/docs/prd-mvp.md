...
- **One usage log, not an event system:** every metered AI request — gateway or direct — is recorded as a single row in one append-only usage log, written in-line by the component already handling that request (the gateway for volunteer traffic; the platform backend for its own Discovery/assistant calls). A row carries surface/actor, project, request id, provider status, the provider's echoed token usage and its computed cost (tokens × the versioned rate card — the provider echoes token counts per response, never dollars), fuel delta, rate-card version, and ledger linkage — metadata only, never bodies. Per-row cost is attribution telemetry; the authoritative dollar spend stays the provider's billed cost per workspace (REQ-006/REQ-009).
- **Tokens are the attribution metric; money is derived.** Attribution is recorded in the provider's echoed token counts — per task, bucket, and category; dollar views derive at read time (token categories priced by the versioned rate card, scaled so the buckets sum to the workspace's billed cost). A rate-card change re-prices a view; it never rewrites recorded attribution. Per-task burn baselines are token-based. Attribution, the NGO burn views, and reconciliation are reads over this one log; nothing subscribes to it and it never gates a request.
...
- Aggregation boundary: the NGO sees burn per deliverable and per NGO-facing category (Discovery, assistant), in cents — derived from token attribution scaled to the billed total — no celebration. Exploration, onboarding, and unattributed appear as their own honest buckets, so total fuel always reconciles to visible lines — spend is never hidden or folded into a task. Per-volunteer-per-task detail and the per-project unattributed-% / binding-failure signals stay coordinator-side. Wide per-task cost variation is expected data, not an anomaly.
