...
| Lovable: `lovable_workspace_setup_reminder` (`matched_pending_fuel` + `lovable_recommended`), `lovable_credits_low`, `lovable_credits_blocked` (escalation tier), `lovable_setup_pending_raised` (volunteer initiated post-kickoff → NGO), `lovable_setup_complete` (blocker auto-resolved → NGO + volunteer), `lovable_skipped_manual` (volunteer chose No → NGO informational), `lovable_decision_auto_skipped` (14-day auto-skip by a pg_cron → `lovable-setup-autoskip` Supabase Edge Function (Deno) per REQ-021, firing via the shared notify() Deno module → NGO + volunteer informational; copy: *"Defaulted to Claude Code only since the volunteer didn't decide on Lovable within 14 days"*) | per event (escalation tier on `lovable_credits_blocked`) | email + in-app | codex round 8 fix — added explicit `lovable_decision_auto_skipped` event |
