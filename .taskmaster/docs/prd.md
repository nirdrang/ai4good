...
- **Batched, not real-time** — admin work that must remain manual (e.g. NGO verification review, REQ-002) is **batched twice daily** (9am + 5pm) rather than processed on-demand. NGOs are told upfront ("verification is reviewed in the twice-daily batch — typically within one business day") so expectations match the SLA. *(Anthropic key creation was the original example here — dead since decision-21: virtual keys mint automatically at kickoff, no ops task, no wait.)* **Lovable setup is NOT an admin task** as of open-item-4 redesign (2026-05-26) — it's volunteer-driven via the `lovable_setup_pending` blocker (REQ-021).
...
ai4good's deliverable is not "a repo" — it is a **deployed, running tool the (non-technical) NGO can continue to evolve itself.** There is **one deliverable model** (decision-23): the durable home is an AI app-builder (**Lovable** in v1) where the NGO evolves the live app via chat after handoff — fixing bugs, adding features, with no developer. The volunteer builds the first version through Claude Code, which orchestrates Lovable.

Discovery (REQ-004) still asks *who maintains the tool after the volunteer leaves* as a **fit check**: a need that requires **ongoing developer maintenance** (custom logic / integrations / no expectation of non-dev chat maintenance) can clear "deployed + running" but by construction cannot satisfy "the NGO evolves it itself," so ai4good does not take it on. Such needs are **declined at Discovery** with plain messaging — ai4good builds tools NGOs can keep evolving themselves via chat; a need that requires ongoing developer maintenance is not a fit — and each decline is recorded for founder review (analytics visibility only; no queue, no waitlist, no re-engagement promise). See REQ-004.
...
- **Primary Metric:** Number of projects reaching `handed_off` **with a verified live deployment URL** (REQ-012 deploy step — live Lovable app + workspace ownership transferred to the NGO).
...
- [ ] At the end of discovery, the NGO is shown: a scope document, a **qualitative complexity tier** (small / medium / large — no dollar estimate in v1, per REQ-004), the project's **`data_sensitivity` tier + the data-handling guideline** for it (REQ-004 — Tier-2 = fixtures-only during build), the **maintenance-fit result** ("you maintain it via chat in Lovable" — a need that requires ongoing developer maintenance is declined at Discovery per REQ-004), a suggested tech stack, and a list of acceptance criteria.
...
- [ ] **On successful funding** (REQ-006 → `in_progress`), the kickoff sequence fires. **`lovable_recommended` is always true and Lovable setup is mandatory (single deliverable model — decision-23)** — no volunteer skip/opt-out. ai4good does not create the repo at kickoff; it auto-raises the `lovable_setup_pending` blocker (REQ-021); NGO + volunteer collaborate via the 11-step checklist to land the repo in `ai4good-projects/<slug>`. **No platform-admin involvement.** Task backlog decomposed from Discovery output into a **draft issue tree that goes live in the project's Linear workspace only after the coordinator's decomposition review** (REQ-026, decisions 20/24(a) — the one human gate in kickoff; no admin ops tasks; repo setup and key issuance never wait on it; reviewer identity = OD-1) — NOT GitHub Issues, which are dev-internal only (REQ-008); virtual keys minted (REQ-009, decision-21); project comment thread opened (REQ-015).
- [ ] Volunteer receives an in-platform "starter kit" with the build guide (Claude Code setup + the Lovable-orchestration guide pointer), plus the pull-based Linear workflow guide (browse briefs → assign → work → PR merge moves status).
...
- Task 3.7: Starter kit page with the build guide (~4h)
...
**I want to** use Claude Code as my single entry point — doing backend/logic/tests directly (metered against project fuel) and orchestrating Lovable via its MCP for the app/UI layer —
...
- [ ] Volunteer receives a Project Starter Kit page + installs the **ai4good Claude Code Skill (REQ-028)**, which primes project context, wires the **Linear MCP** (REQ-026) + the gateway env vars, and sets up the Lovable orchestration.
...
- [ ] **Claude Code orchestrates Lovable (REQ-021 + REQ-028):** the Skill drives Lovable via the `LovableDriver` port (`send_message`/`get_diff`/…), enforcing the per-task Lovable-credit budget cap + audit, with manual dual-rail fallback if Lovable MCP is down. The NGO owns the Lovable workspace (durable post-handoff home).
...
- Task 4.6: Starter Kit page + Skill install guide (~6h)
- Task 4.7: Lovable Status widget + status transitions (~5h)
- Task 4.8: Dual fuel-meter component for project page (~4h)

**Dependencies:** REQ-009 (LLM gateway + virtual keys + fuel ledger), REQ-010 (dual fuel-meter display), REQ-021 (Lovable orchestration), REQ-026 (Linear task rail via the Skill), REQ-028 (Skill orchestration shell).
...
- [ ] Issue status is deterministic: self-assignment marks "in progress" (the commitment signal — the dev's pull); PR merge via Linear's GitHub integration marks "done"; agents comment and self-assign but never move status (detect-and-revert enforces, REQ-026). The working discipline is **pull-then-complete, and the Claude Code Skill (REQ-028) enforces it (decision-24(b))**: the dev pulls an issue (or explicitly enters the exploration bucket) before substantial work — so every metered request carries attribution context — and completion is an explicit dev act: the dev marks work done, and the Skill verifies and drives the merge path that flips status. The project's % complete = `done_p0 / total_p0` from the projection.
...
- [ ] On acceptance: archival of project in ai4good (status `handed_off`); **no repo-transfer step (decision-24(c))** — the repo stays in the platform org permanently, with the NGO admin promoted to `admin` and the volunteer downgraded (ownership is already real: MIT + Lovable two-way sync — fork or export anytime); volunteer's profile gets a **completion credit** (counter +1 + "Shipped first tool" badge on first handoff — no star rating shown publicly).
...
- Task 6.3: Handoff repo permission adjustment (GitHub API; NGO admin promoted to `admin`, volunteer downgraded to `triage`; repo stays in `ai4good-projects` permanently — no transfer step, decision-24(c)) (~4h)
...
- [ ] **Maintenance-fit classification (decided 2026-06-03; single-track per decision-23):** Discovery asks *"after the volunteer is done, who keeps this running and evolving?"* and sets `maintenanceTrack.track`. **`A_ngo_maintains` (non-technical NGO maintains via chat)** → AI builder (Lovable) is the primary deliverable vehicle (REQ-021); this is the only deliverable model ai4good builds. A need that requires **ongoing developer maintenance** (pure-backend, heavy custom logic/integrations, or Tier-2 data that cannot live in Lovable) does **not** get a publishable scope; Discovery **declines it** with plain messaging — *ai4good builds tools NGOs can keep evolving themselves via chat; this need requires ongoing developer maintenance and is not a fit* — and the decline is recorded (the `'B_developer_grade'` classification + rationale persist on the need, plus an `audit_events` entry) for founder review: analytics visibility only — no queue, no waitlist product surface, no re-engagement promise. No build kickoff. The classification gates publishing; every published project takes the REQ-008/021 Lovable provisioning and the REQ-012 deploy/handoff path.
...
- [ ] **Per-turn credit cost is conversation-weighted (decision-11, 2026-06-06; file-weighting removed by decision-24):** each Discovery turn deducts `credits = ceil( (uncached_input_tokens + (discovery_cached_input_weight_bps/10000) × cached_input_tokens + output_tokens) / discovery_tokens_per_credit )` (defaults: cached input at 10%, 10,000 tokens/credit — `platform_settings`), **with reference-file content tokens (REQ-032) excluded from the count — attaching/reading files costs zero credits; the daily allowance is the platform-spend ceiling (decision-24)**. So a **long conversation** (whole context re-sent each turn) costs more credits; **prompt caching the system prompt + conversation prefix makes turns cheaper** (cached tokens count at 10%). Credits are decremented on `ngos.discovery_credits_remaining` from the metered token counts, logged to `audit_events`.
...
- [ ] **Transparency UI (decision-11, the anti-Lovable move):** the Discovery chat shows a **credit gauge** ("Discovery credits: 7 of 10 today") and each turn shows **its** credit cost. **No pre-ingestion file confirm (decision-24):** attaching a file costs zero credits and never interrupts (REQ-032). ai4good copies Lovable's per-action cost display but rejects its opacity / forfeiture-on-cancel / silent-burn — credits are a free grant, never silently removed.
...
interface DiscoveryOutput {
  ...
  // Maintenance-fit classification (decided 2026-06-03 — Item 2; single-track per decision-23). The
  // SELECTOR is "who evolves this after the volunteer leaves," NOT the technology.
  maintenanceTrack: {
    track: 'A_ngo_maintains' | 'B_developer_grade';   // 'A_ngo_maintains' is the only publishable classification; 'B_developer_grade' = fit-check fail → declined at Discovery (decision-23)
    rationale: string;
    // 'A_ngo_maintains': a non-technical NGO staffer will evolve the live app via chat → AI builder (Lovable)
    //   is the PRIMARY deliverable vehicle + durable home; Claude Code is the dev's build/debug rail.
    // 'B_developer_grade' (DECLINED at Discovery — decision-23): custom logic/integrations, special-category
    //   data, or no non-dev chat-maintenance expectation. The platform detects these needs and declines them
    //   instead of publishing a scope; each decline is recorded for founder review (see REQ-004
    //   maintenance-fit classification).
  };

  suggestedLovable: {
    needed: boolean;       // always true under decision-23 (vestigial flag, kept for schema stability)
  ...
  recommendedWorkflow: {
    mode: 'split' | 'claude_code_only';        // Discovery only ever emits 'split' (Lovable UI + Claude Code backend); 'claude_code_only' is never emitted — a pure-backend need fails the maintenance-fit check and is declined at Discovery (decision-23); 'lovable_only' deferred to v1.5 — all matched projects require Anthropic fuel per REQ-006 / REQ-009
    rationale: string;
...
- **Workflow split:** When the project has BOTH UI work AND non-UI work, recommend `mode: 'split'` and list which parts go to which tool. A standalone pure-backend tool has no Lovable app for the NGO to chat-maintain → it fails the maintenance-fit check and is declined at Discovery (decision-23); a *publishable* project is `split`, with Claude Code on the backend rail. **`lovable_only` is NOT a valid v1 mode** — all matched projects require an Anthropic fuel kickoff and use Claude Code at minimum.
- **Maintenance fit (single-track, decision-23):** ask explicitly who will maintain the tool after the volunteer leaves. If a **non-technical NGO staffer** will own ongoing changes → the need fits (Lovable as the durable chat-maintenance home). If a developer will keep maintaining it, or it's a one-off, or it handles special-category data that cannot live in Lovable → the need requires **ongoing developer maintenance**: do not emit a publishable scope; explain plainly that ai4good builds tools NGOs can keep evolving themselves via chat and that this need is not a fit; the decline is recorded for founder review. Default bread-and-butter NGO internal tools (intake forms, CRUD trackers, directories, lightweight dashboards) to a fit; custom-integration / complex-logic / Tier-2-data needs are declined.
...
- The Lovable section (rendered for every project — `suggestedLovable.needed` is always true under decision-23) shows a disclaimer line: *"Lovable is recommended for [reason]. Lovable subscription is paid directly to Lovable — see [lovable.dev/pricing](https://lovable.dev/pricing) for current tiers."*
...
**Scope doc rendering — complexity + maintenance fit + data tier (no budget):** The scope doc shows:
...
>
> **How you'll maintain it: you keep it yourself.** This tool will live in Lovable, where your team can keep evolving it by chat after the volunteer is done — fixing bugs, adding features, no developer needed. (Lovable subscription, ~$25/mo, is paid directly to Lovable; you own the code regardless. See lovable.dev/pricing.)
>
...
| `in_progress` → `open` (abandonment / rematch — REQ-027, added 2026-06-03) | Inactivity job (auto, 21d no branch activity + no Linear issue movement) OR NGO/volunteer manual release | — | **Outbox saga (idempotent):** revoke ex-volunteer repo collab + revoke the project's virtual keys (REQ-009) + remove Linear workspace membership + NGO notice to remove from Lovable workspace (reuses REQ-007 step-2 teardown, minus account-suspend); **remaining fuel STAYS on the project** (Item 3 — non-cash, project-scoped, used by next volunteer); assigned in-progress Linear issues unassigned + returned to the backlog (REQ-026); ex-volunteer application → `released`; flagged `ghosted` vs `released_for_cause`; project re-opens with rematch priority | Idempotent; retried via outbox until clean |
...
3. **GitHub repo**:
   - **(`lovable_recommended` always true — single deliverable model, decision-23):** ai4good does **not** create the repo at kickoff. It auto-raises the `lovable_setup_pending` blocker (REQ-024); NGO + volunteer collaborate via the 11-step checklist in REQ-021 (NGO steps 1-4, volunteer steps 5-9, ai4good auto-validates 10-11); the blocker resolves when the GitHub repo URL + MCP last_seen_at validate, landing the repo in `ai4good-projects/<slug>`. **No platform-admin involvement.** `lovable_enabled` flips on blocker resolution. Repo URL is required before handoff per REQ-012 hard precondition.
4. **Task backlog decomposed** from Discovery output into a draft issue tree that **goes live only after the coordinator's decomposition review (decision-24(a) — the one human gate in kickoff; no admin ops tasks; repo setup and key issuance never wait on it)** → pushed to the project's Linear workspace via API (REQ-026); the Postgres projection fills via webhooks. Follows the pool assignment in step 2.
...
2. **Acknowledgment gate:** Before NGO sees the "Fund this project" CTA, an acknowledgment modal appears restating the key clauses in plain language, with the volunteer's name pre-filled:
   ...
   - The same modal also reminds NGO to set up their Lovable workspace (`suggestedLovable.needed` is always true under decision-23; REQ-021's pre-kickoff banner is hosted in this modal).
...
5. On successful payment → project status → `in_progress`: the `lovable_setup_pending` blocker is raised — the repo arrives via the volunteer-driven Lovable→GitHub setup (REQ-008/REQ-021) — volunteer notified, virtual key minted (REQ-009).
...
**Dependencies:** REQ-001, REQ-002, REQ-004 (Discovery-side `discovery_consumption` draws on the same ledger), REQ-008 (funding raises the Lovable setup blocker; repo lands via REQ-021 setup), REQ-009 (the gateway writes consumption/skim rows inline).
...
- [ ] Filters: skill match, cause, complexity tier, stack, **public vs private** (volunteers who only want public-portfolio work can filter out private projects). *(No "with/without Lovable" facet — every project is Lovable-built, decision-23.)*
...
**Description:** When a project is funded (NGO completes Stripe top-up at `matched_pending_fuel` → `in_progress`), a GitHub repo is provisioned for the code. **Every project takes the Lovable-recommended (`lovable_recommended = true`) path — the single deliverable model (decision-23).** (Verified 2026-05-23 against [Lovable docs](https://docs.lovable.dev/integrations/github) — Lovable's GitHub App creates the repo itself wherever the NGO points it, and Lovable's project-collaborator permissions do NOT propagate to GitHub):

| Case | Who creates the repo | Where it lives | When |
|---|---|---|---|
| **`lovable_recommended = true`** (Lovable-orchestrated — every project) | **Volunteer drives Lovable→GitHub install** (open-item-4 redesign 2026-05-26) — NGO does workspace + admin-invite (steps 1-4 of REQ-021 11-step checklist); volunteer does install + MCP connector + paste-back (steps 5-9); ai4good auto-validates (10-11). No platform-admin involvement. | **Always `ai4good-projects/<slug>`**. Visibility = public/private per `projects.visibility` | Whenever `lovable_setup_pending` blocker resolves — typically same-day if NGO + volunteer move promptly; 7d aging escalation per REQ-024 if either party silent |

**Repo provisioning (always `ai4good-projects` org):** every project's repo is created via the Lovable→GitHub path (REQ-021 11-step; single deliverable model — decision-23). The App install, collaborator roles, and webhooks below apply uniformly to the Lovable-created repo.
...
- At handoff (REQ-012), the repo stays in `ai4good-projects/` permanently — no transfer step (decision-24(c)); NGO admin promoted to `admin`, volunteer downgraded to `triage`

**Lovable path (volunteer-driven via `lovable_setup_pending` blocker per open-item-4 redesign 2026-05-26):**
...
- Volunteer targets `ai4good-projects` org — Lovable creates the repo there with visibility per `projects.visibility`. Volunteer's permission to do this is granted at onboarding via REQ-007 org-membership.
...
- [ ] **Lovable path (the only path — decision-23):** ai4good does NOT pre-create the repo on funding; project transitions to `in_progress` cleanly. At kickoff the `lovable_setup_pending` blocker (REQ-024) auto-raises (no volunteer opt-in — decision-23); NGO+volunteer collaborate via 11-step checklist (NGO does 1-4, volunteer does 5-9 including the Lovable→GitHub install targeting `ai4good-projects/<slug>` with visibility per `projects.visibility`, ai4good auto-validates 10-11) → `github_repo_url` set, `lovable_enabled = true` flips → from that point the repo carries the same App, collaborators, and webhooks as any `ai4good-projects` repo. **No platform-admin involvement.**
- [ ] **Handoff prerequisite (Platform Promise §2 + REQ-012):** for every project, `projects.github_repo_url IS NOT NULL` is a precondition for `in_progress → handoff_pending` transition. For Lovable projects where NGO hasn't yet completed setup, the volunteer's "Ready for Handoff" button is disabled with copy: "Lovable→GitHub setup not yet complete. Ask NGO to complete the Lovable Setup checklist before handoff." This preserves the all-work-ends-up-in-an-OSS-style-repo guarantee even when the visibility is private.
...
- [ ] **Escalation ladder — documented, NOT built in v1** (each rung with its trigger; listed so the decision *not* to surveil is visible):
  - **Layer 2 — reconciliation:** a weekly job joining ledger burn vs GitHub/Linear activity. Phase-aware (early development is legitimately read-heavy/commit-light; "activity" includes branch pushes, issue movement, clone/fetch events). Shape signals only (cadence, session length, capped-every-day pattern) — usable without reading content. *Trigger: the ledger shows real anomalies.* **[DECISION: OD-5 — reconciliation cadence + anomaly thresholds, if/when built]**
...
- [ ] Marketplace default sort is "newest first" (`created_at DESC`). Volunteer can filter by skill, cause, complexity tier. **No "best match" sort in v1** — that's the v1.5 scoring work. *(No "with/without Lovable" facet — every project is Lovable-built, decision-23.)*
...
- [ ] **Hard precondition (Platform Promise §2 + REQ-008):** `projects.github_repo_url IS NOT NULL` — every project ends in an `ai4good-projects/<slug>` repo at handoff; the repo exists once the `lovable_setup_pending` blocker (the REQ-021 Lovable→GitHub setup checklist) has completed. Where the URL is missing, the "Ready for Handoff" button is disabled with copy: "Lovable→GitHub setup not yet complete. Resolve the `lovable_setup_pending` blocker (REQ-021 11-step checklist) before requesting handoff." **No platform-admin escalation** — NGO + volunteer self-resolve via the blocker's checklist + clarifying_question pattern.
...
- [ ] **Deploy-to-running step (decided 2026-06-03 — Goal 1; the deliverable is a *running* tool, not a repo):** the volunteer's final task is to **deploy a working instance and hand over ownership**, and `projects.deployment_url IS NOT NULL` is a handoff precondition:
  - **Lovable (every project):** the app is already deployed and hosted by Lovable; "deploy" = the volunteer **pastes the live Lovable app URL into the handoff form, which WRITES `projects.deployment_url`** (this is the source of `deployment_url` — the precondition above; the step-9 *setup* paste-back captures workspace/repo URLs only, never the deployed-app URL, so without this step a handoff is structurally un-completable) + transfer Lovable workspace ownership to the NGO (NGO owns it from kickoff per REQ-021, so this is a confirmation) + run the **guided-maintenance handoff ritual**: (i) **enable Supabase RLS** (off by default on Lovable-provisioned Supabase — a PII footgun), (ii) **demo chat/plan mode + revert/checkpoint rollback** to the NGO staffer so they can self-maintain without breaking production, (iii) **set a Lovable spend cap / budget alert** so metered edits never blindside a non-technical owner, (iv) confirm two-way GitHub sync is live.
...
- [ ] **Repo handoff:** the project repo remains under `ai4good-projects/` **permanently** with the NGO admin promoted to `admin` and the volunteer downgraded to `triage` — **no transfer step (decision-24(c)):** ownership is already real without one (MIT + Lovable two-way sync — the NGO can fork or export anytime). Private/Tier-2 projects use private *visibility* within `ai4good-projects` in v1 (the separate private org is a v1.5 hardening per REQ-008).
...
**Dependencies:** REQ-008, REQ-006, REQ-021 (Lovable deploy + ritual), REQ-009 (key termination), REQ-026 (p0-done gate + snapshot), REQ-035 (attribution step).

---

#### REQ-021: Lovable Integration — Durable Home + Claude-Code-Orchestrated Build

**Description (rearchitected 2026-06-03 — Item 2):** For **every project** (REQ-004 `maintenanceTrack = A_ngo_maintains` — the single deliverable model, decision-23), **Lovable is the PRIMARY deliverable vehicle and the NGO's durable maintenance home** — after handoff the non-technical NGO evolves the live app via Lovable's chat interface, no developer needed. During the build, however, **Claude Code is the single entry point**: the volunteer works in Claude Code (where the ai4good Skill, fuel metering, and enforcement live), and **Claude Code orchestrates Lovable via Lovable's MCP server** (`send_message`, `get_diff`, `get_workspace`, etc.) for the UI/app layer while doing backend/logic/tests directly. This keeps ai4good's metering + scope + audit authoritative even for Lovable work, and gives the volunteer a single pane of glass. **Needs that require ongoing developer maintenance never reach build — they are declined at Discovery (REQ-004, decision-23).**

**Build-phase orchestration model (the v1 architecture):**
...
- **Primary path + graceful fallback (decided 2026-06-03 — dependency posture):** when Lovable MCP is healthy, Claude Code orchestrates Lovable through the port. If Lovable MCP breaks/changes, the Skill **degrades to the manual dual-rail**: the volunteer drives Lovable directly in the browser, ai4good enforces via the commit-prefix + synced-`ai4good-projects`-repo path. **Builds never go dead from a Lovable API change.**
...
**The legacy "split workflow" framing below remains valid for the manual fallback** (both tools commit to the shared GitHub repo). The default is orchestration-through-Claude-Code, not manual side-by-side.

**Manual split workflow (the fallback when Lovable MCP is unavailable — NOT the primary path):**
...
2. **Pre-kickoff reminder:** When the project enters `matched_pending_fuel`, the platform shows a sticky banner (always shown — `suggestedLovable.needed` is always true under decision-23): *"Set up your Lovable workspace and invite your volunteer there. Lovable bills you directly. Setup guide →"*
...
3. **Setup (volunteer-driven, mediated via blockers + channel messages between NGO and volunteer — open-item-4 redesign 2026-05-26):**
   ...
   **Lovable setup is mandatory at kickoff (decision-23):** every project is Lovable-built (`lovable_recommended` is always true) and Lovable is the deliverable, not an option. At kickoff (`matched_pending_fuel → in_progress`) ai4good **auto-raises** the `lovable_setup_pending` blocker (REQ-024, severity `blocking`) addressed to the NGO. Activity-feed event (via notify(), decision-15): *"🚩 Set up the Lovable workspace and invite the volunteer as workspace admin so they can connect GitHub + the ai4good MCP."* NGO sees an Action-Needed banner; the blocker ages on the normal REQ-024 schedule (48h/7d). **The volunteer "skip Lovable / Claude-Code-only" opt-out and the 14-day auto-skip are removed** — `projects.lovable_skipped_at` is never set and `mode: 'claude_code_only'` is never emitted (decision-23).
...
6. **Handoff:**
   ...
   - NGO retains the Lovable workspace + billing relationship. The GitHub repo lives in `ai4good-projects/<slug>` permanently (per open-item-4 invariant; **no transfer step at handoff — decision-24(c)**; REQ-012 adjusts permissions only: NGO admin promoted to `admin`, volunteer downgraded — ownership is already real via MIT + Lovable's two-way sync). Lovable's GitHub-sync remains pointed at the repo — no Lovable-side reconfig needed.
   - ai4good archives `lovable_workspace_url` for reference; no further state changes

**Acceptance Criteria:**
- [ ] Discovery Agent output schema includes `suggestedLovable: { needed: bool, rationale: string }` block — no dollar estimate fields (REQ-004 consistent). `needed` is always true under decision-23 (vestigial flag, kept for schema stability).
...
- [ ] Project table adds: **two distinct flags** — `lovable_recommended BOOLEAN DEFAULT false` (Discovery's recommendation; **always true — single deliverable model, decision-23**; retained as a flag) AND `lovable_enabled BOOLEAN DEFAULT false` (NGO completed setup; drives status-widget visibility and MCP connector reminders) — plus `lovable_workspace_url TEXT`, `lovable_status TEXT DEFAULT 'not_enabled' CHECK (lovable_status IN ('not_enabled','active','credits_low','blocked'))`.
...
- [ ] **Lovable setup auto-initiated at kickoff (decision-23):** when a project reaches `in_progress` (`lovable_recommended` always true), ai4good auto-raises the `lovable_setup_pending` blocker — no volunteer "use Lovable?" choice. The skip-to-Claude-Code-only opt-out is removed.
- [ ] **`projects.lovable_skipped_at TIMESTAMPTZ`** column retained but **never set** (the skip opt-out is removed — decision-23); kept so existing migrations stay untouched.
- [ ] **`lovable_setup_pending` blocker** auto-raised at kickoff (no volunteer opt-in needed — decision-23); severity `blocking`; resolved by ai4good auto-validation after step 9 of the checklist (URL + MCP `last_seen_at` validation). Added to REQ-024 blocker types + CHECK constraint + unique partial index.
...
- [ ] **ai4good Lovable Setup page** in the platform UI provides (reachable for every project — `lovable_recommended` always true, decision-23):
...
- [ ] Handoff runbook (REQ-012) includes the "NGO removes volunteer from Lovable workspace; ai4good's GitHub App remains on the repo until NGO opts to remove it" step (no ownership-transfer trigger — the repo stays in the platform org, decision-24(c)).
...
- **Agentic loops:** volunteers are free to run loops — the governance stack is request-shaped and loop-agnostic. The repo template ships a skeptical reviewer-agent skill (a default, not a wall); loops must degrade gracefully on MCP auth failure (queue Linear updates locally, surface at session end); **loop PRs are never auto-merged.** Who merges is undecided: **[DECISION: OD-1 — reviewer identity + merge authority per project (coordinator? peer volunteer? agent-assisted with a human click?)]**
...
- **Task binding + degradation (decisions 20/22/24 — the load-bearing change):** the repo's committed `.claude/settings.json` carries the **PostToolUse hook on the Linear assignment MCP call** — it writes the issue identifier to the gitignored, worktree-relative `.a4g/task.md` (REQ-034), which CLAUDE.md imports so the binding rides every gateway request. **The Skill enforces the pull-then-complete flow (decision-24(b)):** before substantial work it requires an explicit choice — pull an issue or explicitly enter the `exploration` bucket — so every metered request carries attribution context (no silent unattributed drift); exploration turning into implementation → it insists on binding the matching issue; completion is an explicit dev act — the dev marks work done, and the Skill verifies and drives the merge path that flips status (REQ-026). **On Linear MCP auth failure the session degrades without stopping:** intended Linear updates queue locally and surface at session end; the binding floors to `unattributed` — the auth-failure floor (REQ-034 ceiling), not a bypass of the discipline.
- **Slash commands:**
...
**The Skill is the orchestration shell (promoted to v1 CORE 2026-06-03 — Item 2).** It is no longer "an onboarding helper with a deferred orchestration layer" — the Skill *is* how Claude Code drives Lovable. The following moved from v1.5 into **v1** (behind the `LovableDriver` port + graceful fallback per REQ-021):
...
- **Triage decision logic:** `/ai4good:work-on <task_key>` reads the task + Discovery's `claudeCodeScope` / `lovableScope` and decides Claude-Code-local vs delegate-to-Lovable, explaining the decision with an override option (`maintenanceTrack` is no longer a runtime input — always `A_ngo_maintains` under decision-23, vestigial). Heuristic: visual UI rendering → Lovable; everything else → Claude Code local.
...
- **NGO consent gate:** project page has an "Allow Skill to orchestrate Lovable" toggle (default ON at kickoff, with cost disclosure; NGO can revoke). Skill checks `get_project_context.lovable_orchestration_allowed` before any Lovable call.
...
- Revoke the ex-volunteer's repo collaborator grant + revoke the project's virtual keys (REQ-009) + remove their Linear workspace membership (REQ-026) + post an NGO notice to remove them from the Lovable workspace — **reuse the AUP per-project teardown (REQ-007 step 2) minus the account suspension.**
...
| Lovable: `lovable_workspace_setup_reminder` (`matched_pending_fuel` + `lovable_recommended`), `lovable_credits_low`, `lovable_credits_blocked` (escalation tier), `lovable_setup_pending_raised` (auto-raised at kickoff → NGO — no longer volunteer-initiated), `lovable_setup_complete` (blocker auto-resolved → NGO + volunteer). *(`lovable_skipped_manual` + `lovable_decision_auto_skipped` removed — the skip opt-out + 14-day auto-skip are gone; single deliverable model, decision-23)* | per event (escalation tier on `lovable_credits_blocked`) | email + in-app | skip events (`lovable_skipped_manual`, `lovable_decision_auto_skipped`) removed (decision-23); `lovable_setup_pending_raised` auto-raised at kickoff |
...
CREATE TABLE projects (
  ...
  -- Data sensitivity + maintenance fit (decided 2026-06-03 — Items 1 + 2; set from Discovery output)
  ...
  maintenance_track TEXT CHECK (maintenance_track IN ('A_ngo_maintains','B_developer_grade')),  -- 'A_ngo_maintains' is the only value that yields a publishable scope; 'B_developer_grade' records a maintenance-fit decline at Discovery (decision-23 — kept for founder-review analytics; the need never publishes)
  ...
  -- Lovable orchestration (REQ-021): NGO-owned workspace; ai4good's backend doesn't bill it against fuel, but the volunteer's Skill meters + caps per-task usage
  -- Two distinct fields, codex round 4 critical fix — were overloaded as one:
  lovable_recommended BOOLEAN DEFAULT false,   -- Discovery flipped this when suggestedLovable.needed = true; always true (single deliverable model, decision-23); drives NGO-setup CTAs
  ...
  lovable_project_id TEXT,                      -- Lovable's internal project ID; REQUIRED for Claude-Code-orchestrates-Lovable (REQ-021 + REQ-028 LovableDriver port) so the volunteer's Claude Code passes it to Lovable MCP `send_message(project_id=...)`. Captured by volunteer at step 9 of lovable_setup_pending blocker checklist.
  lovable_skipped_at TIMESTAMPTZ,               -- never set: the volunteer skip opt-out is removed (decision-23); column retained so existing migrations stay untouched (was: volunteer chose Skip Lovable, project proceeds Claude-Code-only)
...
- Task 2.9: GitHub App registration + repo creation on funding (Claude Code path only; Lovable repo URL captured separately per REQ-008) — Medium (10h) **(superseded by decisions 19/23: no repo creation on funding — the repo arrives via the volunteer-driven Lovable→GitHub setup, REQ-021; the GitHub App registration + org-wide install is still needed)**
...
- Task 2.16: Lovable surfaces (REQ-021: manual status widget + Lovable Setup page — post-decision-20, no connector/token panels — + Knowledge-field snippet docs; the active orchestration logic lives in the Skill, Task 2.18; **no inbound email parser in v1**) — Small (5h)
...
- Task 2.24: **Decision-8 unified-fuel + decision-11 Discovery CREDITS** — `ngos.discovery_credits_remaining` (credits, not cents) + `platform_settings` credit params + `unverified→verified` 500-refill + `profiles.email_verified_at` sync trigger + Stripe Checkout reachable from `draft`/`discovery_in_progress` + `discovery_consumption` ledger kind + **conversation-weighted per-turn credit cost (caching discount; file reads cost zero credits — decision-24) + credit gauge + per-turn cost UI** + credits-exhausted composer states (verify-CTA / top-up-CTA) + first-funding hard-ack modal (REQ-002/004/006/032, Promise §3/§9) — Medium (13h)
...
10. **Hosted production environment for built tools** — Hosting is Lovable's: the deliverable ships as a live Lovable app (REQ-012); ai4good never operates the tools' infra.
...
14. **ai4good-funded Lovable infrastructure** — ai4good does not subscribe to Lovable on behalf of NGOs and does not bill Lovable consumption against fuel; the NGO owns + pays for the Lovable workspace directly. **However (post-2026-06-03 pivot), the ai4good Skill DOES meter + cap Lovable usage per task and surface the NGO's credit balance via `get_workspace` during build orchestration (REQ-021/028)** — "doesn't meter" applied to the old side-track model and is no longer true. Lovable is the primary deliverable vehicle, not a side track.
...
- **Decision-11 (2026-06-06) — Two-layer money: $-based funded fuel + a context-weighted Discovery CREDIT system.** Validated by a credit-vs-money research+tournament workflow (Lovable + v0/Replit/Cursor/Devin + SaaS prepaid-credit/escheatment/ASC-606). Founder calls: **no subscription; the 15% skim does not change; funded fuel stays real-dollar-pegged (status quo, Option A); but the FREE pre-fuel Discovery phase becomes an abstract CREDIT system where credit burn scales with context weight.** This mirrors Lovable's two-layer model (abstract credits where cost variance is absorbed = Discovery; real dollars where cost is metered + attributable = funded fuel), and the regulatory bonus: Discovery credits are a **free grant the NGO never purchases**, so — unlike *purchased* credits — they carry zero stored-value / money-transmitter / escheatment exposure.
  ...
  - **Context weight drives the burn (the founder's ask):** per Discovery turn, credits deducted = `ceil( (uncached_input_tokens + 0.1 × cached_input_tokens + output_tokens) / discovery_tokens_per_credit )`. So a **long conversation** (the whole context re-sent each turn) and a **big uploaded reference file** (REQ-032 — rides in context every turn) cost more credits *(file-weighting removed by decision-24)*, and **prompt caching makes turns cheaper** (cached tokens count at 10%). Discovery output regeneration (≤3×, REQ-004) and system-error retries are **free** (avoids the "retries silently burn credits" complaint the research flagged at Lovable/Cursor).
  - **Transparency (the anti-Lovable move):** the Discovery chat shows a **credit gauge** ("Discovery credits: 73 of 100"), each turn shows **its** credit cost, and a file upload shows **"≈ N credits to include this file" BEFORE** the agent ingests it *(the pre-ingestion per-file cost display is removed by decision-24 — files cost zero credits and never interrupt)*. ai4good copies Lovable's per-action cost UI but rejects its opacity / forfeiture-on-cancel / silent-burn — credits are a free grant, never silently removed.
...
- **Decision-23 (2026-07-07) — Single-track platform: Track B removed altogether (supersedes decision-19's "deferred post-v1 + waitlist" posture).** The track taxonomy disappears from all forward-looking text; there is **one deliverable model**: a deployed tool the NGO maintains itself via chat (AI-builder durable home; Lovable in v1), built by a volunteer through Claude Code orchestration. The developer-grade **waitlist is deleted**. Discovery still performs the fit check (the maintainability classification stays — the platform must detect needs that require ongoing developer maintenance), but a non-fit need is **declined at Discovery** with plain messaging (ai4good builds tools NGOs can keep evolving themselves via chat; this need requires ongoing developer maintenance and is not a fit). Each decline is recorded for founder review — analytics visibility only: no queue, no waitlist product surface, no re-engagement promise.
- **Decision-24 (2026-07-07, founder review batch 2):** (a) kickoff decomposition coordinator-gated — one human gate, no admin ops tasks; the draft issue tree goes live only after the coordinator's decomposition review, and repo setup + key issuance never wait on it; (b) the Claude Code Skill enforces pull-then-complete (explicit task-or-exploration binding before substantial work; completion is an explicit dev act driving the merge); (c) handoff repo-transfer step removed — repo stays in the platform org permanently; (d) Discovery file attachment/reads cost zero credits, no confirm interruption — the daily allowance is the spend ceiling.
...
- REQ-021 (Lovable = the primary deliverable vehicle; Claude Code orchestrates it via the `LovableDriver` port over Lovable MCP with per-task credit cap + audit + manual dual-rail fallback; volunteer-driven setup; manual status widget retained; **inbound email parser deferred to v1.5**; **single-track per decision-23 — needs that require ongoing developer maintenance are declined at Discovery**)
...
**Handoff:**
- **REQ-012 minimal** (Automated checklist + sign-off; repo permission adjustment — no transfer step, the repo stays in the platform org permanently (decision-24); virtual-key termination + Linear membership removal + tree snapshot; **includes the REQ-035 attribution step — decision-22**. **Tip flow deferred to v1.5.**)
...
| **Replit as a 2nd sanctioned builder platform** (decided 2026-06-03 — Item 2) | A project needs clean post-build ownership transfer that Lovable can't do, or Lovable reliability falters | ~30h (per-platform handoff playbook + PII review + integration) |
...
### v1 design rationale: Claude Code orchestrates Lovable (now the v1 architecture — promoted 2026-06-03)

**This is now v1 (REQ-021 + REQ-028), not a future direction.** For every project the volunteer's local Claude Code runs the ai4good Skill and is configured with two MCP servers — the **Linear MCP** (tasks — decision-20; committed `.mcp.json`, volunteer OAuth) and `mcp.lovable.dev` (Lovable, OAuth-user-scoped, Research Preview) — plus ai4good REST for context/blockers/comments, and orchestrates Lovable behind the `LovableDriver` port: pulls the next Linear issue, decides Lovable vs local, prompts Lovable via `send_message`, reads `get_diff`, runs tests, iterates; **status lands via the PR merge** (Linear GitHub integration), never by agent say-so. If Lovable MCP breaks, the port falls back to manual dual-rail. This section captures the rationale + the concrete Lovable tool surface; the *requirements* live in REQ-021/REQ-028.
...
- **Research-Preview risk handling:** the `LovableDriver` anti-corruption port + manual dual-rail fallback (REQ-021) mean a Lovable MCP breaking change degrades gracefully — builds never go dead. **Checkpoint USER-TEST** validates orchestration on 1-2 real projects in pilot.
...
| 7 | ~~**Deployment ownership post-handoff**~~ | **Decided 2026-06-03** | Item 2 / Goal 1: the deliverable is a *deployed running tool*. Track A → Lovable hosts it + NGO self-maintains via chat (NGO owns the workspace + ~$25/mo to Lovable, disclosed); Track A is the only track v1 builds. (Track B → free-tier host on the NGO's account + credentials handoff is deferred to post-v1 — decision-19.) *(Superseded by decision-23 — Track B removed; such needs are declined at Discovery.)* ai4good never operates infra. `deployment_url` is a handoff precondition (REQ-012); 30-day-alive tracked. |
...
- **Status: Researched 2026-05-16; re-scoped 2026-06-03 by the Track-A pivot.** The question splits into two distinct integrations that the pre-pivot text conflated:
  1. **Client-side, in v1 (SHIPS):** During build orchestration the *volunteer's* Claude Code Skill calls `mcp.lovable.dev` `get_workspace` (under the volunteer's own OAuth, behind the consent gate + `LovableDriver` port) to read the NGO's credit balance and enforce the per-task budget cap (REQ-021/028). This is per-session, user-scoped, and consented — not backend polling.
...
- [ ] One full volunteer journey: signup (GitHub/Google/email) → browse → apply (link GitHub at this step if not yet linked, per REQ-007) → NGO accepts → wait for NGO fund → in_progress (virtual key live instantly; Linear backlog after coordinator review; repo after the Lovable setup checklist completes)
