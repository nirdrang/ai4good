...
- [ ] Stripe webhook (`checkout.session.completed`) creates **a single `fuel_transactions` row** for the top-up — no skim split at top-up time:
...
- [ ] Background job nightly: any `matched_pending_fuel` project older than 7 days → revert to `open` + notify both parties. *(Decision-17: no `paused` state in v1, so the job needs no freeze-clock guard — it acts on every `matched_pending_fuel` project by status alone.)*
...
-- Worker: SELECT ... FOR UPDATE SKIP LOCKED; idempotent handlers; terminal failures → status='failed_dlq' + an `incident` ops_task (DLQ).
...
- On funding, ai4good auto-creates `ai4good-projects/<slug>` via the App's repo-creation API; visibility set per `projects.visibility` (`public_mit` → public repo + LICENSE seeded as MIT; `private` → private repo)
...
**Acceptance Criteria:**
- [ ] Platform stores the GitHub App's **installation ID + private key** (NOT long-lived installation tokens — codex round 6 nit fix). Backend mints short-lived installation tokens on demand via the GitHub App JWT flow (~1h TTL; auto-refreshed). All GitHub API calls use these per-request tokens, not user PATs.
- [ ] **One-time platform setup at platform launch (single ops task; never repeated):**
  - **ai4good's own GitHub App** installed on `ai4good-projects` org with permissions: `Repository administration: write` + `Contents: write` + `Pull requests: write` + `Workflows: write` + **`Organization members: write`** (codex round 6 + open-item-4 redesign — needed so ai4good's backend can add/remove volunteer accounts as org members without per-project admin labor; narrower than `Organization administration: write` — covers membership add/remove/role-change only, NOT org settings/billing/teams; risk mitigations: audit_events log + nightly reconciliation job + max-membership-lifetime auto-revoke). Token stored in Supabase Vault per NFR Security.
  ...
  - **Org permissions configured (preserves private-repo confidentiality):**
    ...
    - **App credential least-privilege + rotation:** the GitHub App credential is scoped to the minimum needed, rotated on a schedule, and a **break-glass org-compromise runbook** is documented (REQ-030).
...
1. **Automated:** Platform calls Anthropic Admin API `POST /v1/organizations/workspaces` to create a workspace named `ai4good-<project-slug>`. Workspace ID stored on the `projects` row.
...
- [ ] Ops queue UI at `/admin/anthropic-ops` lists pending key-creation tasks with workspace ID, project name, fuel budget; admin marks task done after pasting key.
...
- [ ] Fuel-low alerts: at 20% balance → NGO email + `fuel_topup_needed` blocker (warning, REQ-024); at 5% → in-app warning to volunteer; at 0% → key programmatically deactivated via Admin API (`POST /v1/organizations/api_keys/{id}` with `status: 'inactive'`) + `fuel_topup_needed` blocker (blocking).
...
- Task 9.2: Workspace auto-creation on `in_progress` transition (~4h)
...
- Task 9.6: Usage API polling worker (one global call grouped by workspace_id per the AC — NOT per-workspace fan-out; codex round 8 nit fix) (~6h)
- Task 9.7: Fuel-low alerting + blocker integration (REQ-024) (~4h)
- Task 9.8: Programmatic key deactivation at fuel-zero (~3h)
...
- [ ] **Cadence stats panel** (computed from GitHub commits via webhooks + hourly background pull — data ingestion only; no GitHub UI surfaced):
...
- [ ] Aging job runs hourly; fires `blocker_aging_48h` and `blocker_aging_7d_escalation` events.
...
- Task 24.6: Aging-blocker job (hourly cron) + notifications (~5h)
...
- `GET /api/projects/:id/tasks/export.json` → returns full task tree in a TaskMaster-compatible JSON shape
...
- [ ] On project `in_progress` transition, the **bootstrap-from-Discovery server action** generates the initial `tasks.json` from Discovery's `userStories[]` + `acceptanceCriteria[]` and **commits it to the project's GitHub repo** as the first commit on `main` (ai4good GitHub App authorship). Idempotent — re-run is a no-op if any `source='discovery'` rows are already present in the projection.
- [ ] **GitHub webhook receiver** for `push` events on `ai4good-projects/*` parses the commit's file list; if `.taskmaster/tasks/tasks.json` is modified, fetches the post-push file via the GitHub App API, diffs against the current projection, upserts changed rows into `project_tasks`. Records each ingest as one `audit_events` row with `git_commit_sha`, `actor_source = 'git_push'`, `delivery_id` for de-dup.
- [ ] **Push retry / catch-up:** if the webhook receiver was down when a push happened, GitHub's redelivery picks it up; additionally, a low-frequency cron polls each active project's `main` HEAD SHA and reconciles any missed pushes against the projection.
...
- [ ] Volunteer can raise task-anchored clarification blockers via "❓ Raise concern on this task" — both from the ai4good UI and from the Skill's `/ai4good:raise-blocker --task=X` command. Creates a `clarifying_question` blocker (REQ-024) with `task_id` set, via the **REST API** (`POST /api/projects/:id/blockers`). Banner anchors to the task; Q&A persists on the task and in the Clarifications log on resolution.
...
- [ ] **TaskMaster-compatible export endpoint** dropped — the project repo IS the export. `git clone <repo>; cat .taskmaster/tasks/tasks.json` is the canonical export. Optional convenience: `GET /api/projects/:id/tasks/tasks.json` proxies to the repo's current `main` HEAD via the GitHub App, returning the raw `tasks.json` (read-only, no auth other than project membership for private projects).
...
- Task 26.2: **Bootstrap-from-Discovery server action** — read scope doc → generate `tasks.json` → commit to repo via GitHub App (~5h)
...
- Task 26.5: **Cron reconciliation worker** — periodic poll of each active repo's `main` HEAD SHA for missed webhooks; reconciles diffs (~2h)
- Task 26.6: NGO REST API — `POST /tasks/:task_key/comments`, `POST /blockers`, `GET /context`, `GET /tasks/tasks.json` (proxy); per-project bearer-token auth (~5h)
...
- **Slash commands:**
  - `/ai4good:next-task` → calls **local** `task-master next` (TaskMaster CLI), picks highest-priority unblocked task, shows full context (description, related blockers from REST `GET /api/projects/:id/blockers`, recent commits from local git)
  - `/ai4good:fuel` → calls `GET /api/projects/:id/context` (REST), shows current fuel balance + recent burn rate + projected runway
  - `/ai4good:blockers` → calls `GET /api/projects/:id/blockers` (REST), shows unresolved blockers + suggested actions per type
  - `/ai4good:raise-blocker --task=X` → calls `POST /api/projects/:id/blockers` (REST) with `task_id` populated; creates a `clarifying_question` blocker per REQ-024
...
- **Audit reporting:** every Lovable call logged via the ai4good **REST API** (`POST /api/projects/:id/audit-events`) with a cost estimate, surfacing NGO Lovable burn rate on the admin + NGO dashboards.
...
**Acceptance Criteria:**
- [ ] Inactivity job (hourly) computes inactivity **only for projects currently in `in_progress`** (skips `handoff_pending` and every other status — a plain status filter, decision-17) from commits + PM-task transitions; fires `inactivity_reminder` at 14d, auto-releases at 21d.
...
**Acceptance Criteria:**
- [ ] **`job_runs` heartbeat table** — every scheduled job (5-min usage poll, daily reconciliation, nightly anomaly/expiry, hourly aging/cadence/inactivity, outbox drain, continuous base-permission assertion, 30-day-alive check) records start/finish; an **overdue-run pager** fires if a job hasn't completed within its expected interval.
...
| Lovable: `lovable_workspace_setup_reminder` (`matched_pending_fuel` + `lovable_recommended`), `lovable_credits_low`, `lovable_credits_blocked` (escalation tier), `lovable_setup_pending_raised` (volunteer initiated post-kickoff → NGO), `lovable_setup_complete` (blocker auto-resolved → NGO + volunteer), `lovable_skipped_manual` (volunteer chose No → NGO informational), `lovable_decision_auto_skipped` (14-day nightly job auto-skip per REQ-021 → NGO + volunteer informational; copy: *"Defaulted to Claude Code only since the volunteer didn't decide on Lovable within 14 days"*) | per event (escalation tier on `lovable_credits_blocked`) | email + in-app | codex round 8 fix — added explicit `lovable_decision_auto_skipped` event |
...
- **Response Time:**
  - Marketplace page: < 500ms p95 (with Next.js streaming).
...
- **Throughput:**
  - Up to 1000 concurrent marketplace viewers in year 1 (well within Vercel + Supabase free/pro tiers).
  - Up to 50 concurrent discovery conversations (Anthropic API quota-bounded).
- **Resource Usage:**
  - Stay within Vercel Pro + Supabase Pro tier for year 1 (~$45/mo).
...
- **Secrets:** All API keys (Stripe, Anthropic, GitHub) stored in Supabase Vault / Vercel env; never logged.
...
- **Horizontal scaling:** Vercel auto-scaling for the Next.js app; Supabase provides managed Postgres scaling.
- **Heavy work** (usage polling, webhook fanout) runs on **Supabase Edge Functions** or **a small worker on Fly.io** (TBD — see Open Questions).
...
1. **Next.js 14 App (Vercel):** Server Components + Server Actions for most reads; Route Handlers for webhooks; Edge runtime for marketplace.
...
6. **Worker layer:** Either Supabase Edge Functions (preferred for simplicity) or a Fly.io worker if longer-running jobs prove necessary.
...
```
POST /api/discovery/{conversationId}/message
...
- **Frontend:** Next.js 14 (App Router, Server Components, Server Actions), React 18, Tailwind CSS, shadcn/ui components, TanStack Query (where client-side caching is needed).
- **Backend:** Next.js Route Handlers (Node + Edge), Supabase Postgres, Supabase Auth, Supabase Storage (verification docs), Supabase Realtime. **Cloudflare R2** (S3-compatible object store, zero egress) for **project need attachments** (REQ-032), accessed only via signed URLs from an ai4good edge function.
...
- **Infra:** Vercel (app), Supabase (data + auth), Upstash Redis (rate limits + caches).
- **Observability:** Sentry (errors), Axiom or Logflare (logs), Vercel Analytics.
...
- **E2E:** Playwright. Two scripted journeys: (a) NGO end-to-end happy path, (b) Volunteer end-to-end happy path. Run on staging on every deploy.
- **Performance:** k6 smoke on marketplace + discovery start. Target: marketplace p95 < 500ms at 100 RPS.
- **Security:** Snyk or GitHub Advanced Security on dependencies; manual review of RLS policies; OWASP ZAP baseline scan before launch.
...
- Task 1.2: Next.js app scaffold (App Router, Tailwind, shadcn/ui) — Small (4h)
...
- Task 2.10: **Bootstrap-from-Discovery + NGO REST API** (REQ-026 decision-9 — server action commits initial `tasks.json` to repo on project `in_progress` via GitHub App; REST endpoints `POST /tasks/:task_key/comments`, `POST /blockers`, `GET /context`, `GET /tasks/tasks.json` proxy; per-project bearer-token auth) — Medium (9h)
- Task 2.11: **GitHub webhook receiver + Postgres projection worker** (REQ-026 decision-9 — push event filter, fetch updated `tasks.json`, diff against `project_tasks`, upsert; cron reconciliation for missed webhooks; idempotent via `delivery_id`) — Medium (8h)
...
- Task 2.20: Ledger integrity — pair-sum **control-total** nightly job (every entry balances) + Stripe dispute/`chargeback` webhook handler + `chargeback_writeoff` + `reserve_floor_cents` + `first_fund_cap_cents` enforcement (REQ-006) — Medium (10h)
...
**Validation Checkpoint 2:** End-to-end happy path works in staging — one real NGO + one volunteer can complete a project **through to a deployed, reachable `deployment_url`** (REQ-012 deploy step is a handoff precondition, not optional), with the handoff ritual (RLS-on, revert demo, two-way GitHub, spend cap) executed.
...
- Task 4.8: Performance pass (caching, edge runtime, image opt) — Medium (8h)
...
1. **ai4good AI Proxy (dev-facing token + per-request audit + instant rotation + 9-layer enforcement)** — Moved from v2 to **v1.5 priority** (was originally framed as v2 enforcement; reframed after architectural insight). The proxy sits between Claude Code and `api.anthropic.com`, issuing project-scoped tokens (`ai4g_*`) to volunteers instead of exposing the real Anthropic key. Volunteers configure `ANTHROPIC_BASE_URL=https://proxy.ai4good.dev/projects/<id>/v1` + `ANTHROPIC_API_KEY=<ai4g token>`; vanilla Claude Code works unchanged.
   ...
   **Effort estimate:** ~80h for v1.5 proxy with the 9-layer enforcement model. Built on Cloudflare Workers / Vercel Edge for low-latency global edge presence.
...
24. **Decision-10 v1.5 deferrals (2026-06-06; founder-approved; gate = "before organic/EU signup").** The task-tree simplification pass deferred the following out of the concierge v1 surface; each is logged with its reinstatement trigger and the public-beta launch checklist (REQ-029 / task #56) carries the hard gate:
    - **Rate-limiting / Upstash** (task #47, removed) — v1 ceiling is a Vercel WAF rule + per-surface caps (Discovery caps owned by REQ-004/decision-8; Apply 5/min via the apply unique index; auth via Supabase per-IP throttle). Removes Upstash from the v1 surface entirely. Reinstate full rate-limiting before organic signup.
    ...
    - **Upstash cache + k6/Lighthouse CI gates** (performance right-size) — v1 keeps next/image + ISR + the Supabase pooler.
...
- **Decision-9 (2026-06-06) — Git-as-truth for the PM tree (REQ-026 re-scope):**
  ...
  - **Bootstrap from Discovery becomes a server-side git commit** at project funding: ai4good reads the scope doc, generates the initial `tasks.json`, commits it to the new repo. Volunteer takes over from there.
...
- **Decision-10 (2026-06-06) — Task-tree simplification pass (architecture + simplicity tournament).** A 6-lens survey + per-candidate approach tournament (3 approaches each, judged on architecture + simplicity, capability-preserving) produced six accepted simplifications. These are folded into the affected REQs (each carries a "› Decision-10" override callout). Net ≈ **−80h** off the post-decision-9 baseline (rank 1 already counted under decision-9), landing v1 at **≈ ~385h core / ~480h buffered**. The reshapes touch the *implementation shape*, not v1 capabilities — every preserved REQ AC is intact; only genuinely-v1.5 work is deferred.
  ...
  - **(Rank 6 — founder-approved) Defer the public-beta hardening slice.** To v1.5 (gated "before organic/EU signup", logged in the v1.5 backlog): **Rate-limiting/Upstash** (v1 ceiling = a Vercel WAF rule + per-surface caps), **Gradual-rollout** (allow-list + waitlist suffices), **Analytics dashboard** (Goals 1-5 met by 3-4 Postgres VIEWs). Right-sized in place: **Content moderation (REQ-031)** → keep secret-scanning + push-protection only, defer report/takedown UI + abuse queue; **Stripe Tax** → keep `automatic_tax` + hosted invoices, defer multi-jurisdiction registration; **GDPR** → keep consent + sub-processor list + a *manual* erasure runbook, defer self-serve erasure UI; **Performance** → keep next/image + ISR + Supabase pooler, defer Upstash cache + k6/Lighthouse gates.
...
- **Current Status:** Edge Fns are lighter-weight but have time limits; long usage polls + webhook fanout may exceed them.
- **Options:** (A) Supabase Edge Fns only, (B) Fly.io worker, (C) Vercel Cron + serverless.
- **Owner:** Tech lead.
...
| Stripe webhook delivery failure (fuel not credited) | Low | High | **High** | Stripe handles retries; we make webhook handler idempotent; reconciliation job nightly | Manual reconciliation tool for platform admin |
...
18. Bootstrap-from-Discovery server action + NGO REST API (REQ-026 decision-9 — generate initial `tasks.json`, commit to repo via GitHub App on `in_progress`; REST endpoints for comments / blockers / context / tasks.json proxy; per-project bearer-token auth) (9h)
19. GitHub webhook receiver + Postgres projection worker (REQ-026 decision-9 — push event filter, fetch updated `tasks.json`, diff against `project_tasks`, upsert; cron reconciliation for missed webhooks; idempotent via `delivery_id`) (8h)
...
28. Ledger integrity — pair-sum **control-total** nightly job + Stripe dispute/`chargeback` webhook handler + `chargeback_writeoff` + `reserve_floor_cents` + `first_fund_cap_cents` enforcement (REQ-006) (10h)
