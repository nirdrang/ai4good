...
> **[d82 AMENDED — two-tree model, 2026-07-19]:** the gate now pushes the decomposition to the DEV TREE (each dev item related to its PM requirement); kickoff seeds the PM tree (scope requirement items + the bootstrap item) and NOTHING on the dev tree pre-gate; the bootstrap is a PM-tree item, so requirement-level attribution binds it.
...
- **AT-036.01 (P0)** — Given kickoff completes, When the Linear workspace is read, Then the PM TREE carries the scope-doc requirement items plus the bootstrap "Author the project PRD" item, and the DEV TREE is EMPTY — no decomposition issue exists anywhere before the gate passes. [d82] [cross: AT-026.06/08 own the seeding mechanics]
- **AT-036.02 (P0)** — Given the volunteer authors the PRD, When the work is inspected, Then the PRD file lives in the project repo; the volunteer FIRST pulls the bootstrap item through the Skill — the pull alone moves it to In Progress and establishes the binding — and the authoring session's metered requests then carry bootstrap-bound attribution rows; a PRE-PULL binding attempt does not resolve and records unattributed (probe) [d82v: pull-before-bind] (token telemetry), AND the provider-authoritative billed usage / project-fuel balance shows a corresponding before/after change for the authoring requests — the two meters proven separately. [cx r2: the two-meter correction applied to authoring too — a usage row cannot prove fuel billing] [cross: AT-009.08/AT-034.01/AT-006]
...
- **AT-036.06 (P0)** — Given THREE controlled end-to-end fixtures — a score above, exactly AT, and below the configured threshold (controlled scorer results), When the volunteer performs ONLY the mark-ready action in each, Then the scorer runs and its score event AUTOMATICALLY drives the correct branch with no separate gate invocation: above and at-threshold end with the DEV tree decomposed and pushed AND the bootstrap item moved to Done through the verified-completion path (the gate pass IS its completion evidence, REQ-036); using TWO distinguishable PM requirements + PRD story/AC sentinels, every generated dev item maps to its CORRECT PM requirement — none orphaned, none cross-related — and the PM tree is otherwise unchanged [d82v: bootstrap outcome + correct-mapping fixtures]; below ends with NO dev-tree item created, the bootstrap still In Progress, a gap report produced, and the PRD phase continuing; and the PASSING fixture's PRD carries traceable PRD-specific story/AC wording (absent from the Discovery scope doc — sentinel phrasing) that the pushed tree derives from — the backlog decomposes from the PRD artifact, never from Discovery directly. [cx: end-to-end from mark-ready] [cx r2: at-threshold given its own controlled fixture; PRD-derivation sentinel added — Discovery is a scope contract, not a task source] [cross: AT-026.08/09]
...
|---|---|
| Kickoff seeds the PM tree (scope items + bootstrap); dev tree empty pre-gate | 01 [d82] |
...
| Gate END-TO-END from mark-ready alone: above/AT/below fixtures; DEV tree derives from the PRD artifact w/ PM relations (Discovery = scope contract, not task source) | 06 [cx r2] |
