...
- **AT-005.5.47 (P0)** — Given every actor-restricted transition — NGO-only: start Discovery, confirm its current brief, publish, unpublish, cancel; volunteer-only: match consent; FOUNDER-only: triage approve/return/decline (REQ-023 assigns all three to the founder-reviewer, not any admin); manual release: allowed to the owning NGO and the assigned volunteer, each with a required reason (REQ-027 — "either party") — When each is attempted by every wrong actor (including a DIFFERENT NGO's account and unauthenticated callers), Then each attempt is rejected with the state unchanged and no side effect fired. [cx: added] [cx r2] [d74: approve added to the founder-only set]
...
- **AT-005.5.09 (P0)** — Given no eligible free turn and no available fuel, starting Discovery is blocked. Name fuel checkout and the next daily reset only when beta capacity remains. Preserve the draft. [d92]
...
- **AT-005.5.11 (P0)** — Given valid Discovery output, the project remains discovery_in_progress until the owning NGO confirms the current brief revision. Confirmation records actor, revision, and timestamp and advances to scoped. The AI, another NGO, an unauthenticated actor, or a stale revision cannot approve. Edits invalidate approval of the previous revision. [d92]
...
- **AT-005.5.49 (P0)** — Given invalid output followed by a valid retry within the bound, no admin escalation occurs. The project remains discovery_in_progress until the NGO confirms the valid revision. Then it advances to scoped. [d92]
...
- **AT-005.5.15 (P0)** — Given a project in `triage`, When the founder-reviewer approves, Then the project moves to `open`; and no automated path to `open` exists anywhere (probe: the advisory pass cannot transition state). [d74: reviewer approval replaces screener auto-approve]
- **AT-005.5.16 (P0)** — Given ANY publish, When it lands, Then a founder review-queue entry exists with the advisory evidence attached and the project remains in `triage` until the decision. [d74: universal queueing replaces non-decided routing]
- **AT-005.5.17 (P0)** — Given a review decision to return, When it lands, Then the project is `scoped` with a reason note visible to the NGO, and a subsequent edit + republish re-enters review (a new queue entry + advisory pass recorded). [d74: wording]
- **AT-005.5.18 (P0)** — Given a review decision to cancel (a need editing cannot fix), When it lands, Then the project is `cancelled` and terminal.
- **AT-005.5.50 (P0)** — Given a review-queue case classified as fixable by editing, When the founder acts on it, Then the terminal-cancel action is unavailable for that classification and the return-to-`scoped` path (with reason note) is — `cancelled` is only for needs editing cannot fix. [cx: added]
- **AT-005.5.19 (P0)** — Given projects of every tier in `triage` — clean Tier-1 and Tier-2 alike (two fixtures), When each awaits review, Then NEITHER reaches `open` without a recorded human approval — the universal human-review invariant. [d74: Tier-2-never-auto generalized to everything-never-auto]
...
- **AT-005.5.55 (P0)** — Given an `open` candidate-bearing project, When a match is attempted that is (a) not admin-created or (b) with a volunteer outside the candidate pool, Then each attempt is rejected with the project still `open` and no consent side effects; a valid admin match from the pool succeeds. [cx r2: added — .20 assumed a valid match into existence] [d68: the pre-window timing case dropped — the candidacy window was removed from the PRD] [cross: REQ-007]
...
| scoped → triage on Publish (vetted only); founder approve → open (no automated path); universal review queue; return/cancel (cancel ONLY for unfixable); republish re-enters; nothing-ever-auto invariant | 13–19, 50 [cx] [d74] |
| open → matched_pending_fuel on consent; admin-created from the pool (negatives; window removed d68); first-match disclaimer at consent click (GitHub pre-linked); no NGO approve/decline; one match at a time (closed history never blocks) | 20, 21, 55, 56, 57 [cx r2] |
