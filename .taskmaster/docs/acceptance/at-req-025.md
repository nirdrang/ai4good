...
- **AT-025.02 (P0)** — Given the volunteer accepts in-thread AND creates the correctly linked task, Then — and only then — the addition is in scope; and the chronology proves order: the acceptance and linked task timestamps precede the first metered provider usage for that work (a known material-work fixture, judged on the authoritative request/usage chronology — not on attribution rows, which REQ-034 keeps soft). [cx r2: oracle moved from attributed-burn to metered-usage chronology] [cross: REQ-026/034]
- **AT-025.10 (P0)** — Given three defective variants — an in-thread acceptance WITHOUT a volunteer-created linked task, a volunteer acceptance OUTSIDE the project thread, and an in-thread acceptance paired with a task linked to the WRONG project — When scope is probed, Then each addition is still out of scope — the conjunction requires the in-thread acceptance AND the correctly linked task. [cx: acceptance-without-task] [cx r2: outside-thread + wrong-project variants added]
