...
- **AT-028.09 (P0)** — Given the dev invokes the complete flow on the pulled requirement, When the Skill acts, Then it verifies the requirement's completion evidence (dev-tree work merged, acceptance evidenced) and records the verified completion that flips the PM item — completion is an explicit dev act at the PM level; dev leaves flip on merge with no Skill act (probe: no Skill path exists to flip a dev leaf); and Given three DEFECTIVE variants — a linked dev item still unmerged, acceptance evidence absent, and evidence recorded against a DIFFERENT requirement's pull — Then each leaves the pulled requirement In Progress with the failed gate named. [d82v: completion negatives] [cross: AT-026.17]
...
- **AT-028.11 (P0)** — Given the pick-next helper, When invoked, Then it offers the highest-priority unblocked PM REQUIREMENT with its full context, and on confirm — atomically — records the PULL, assigns the volunteer, authorizes In Progress, and establishes that requirement's worktree binding; on decline, NONE of those occur. [d82v: the pull is authority + record + binding in one act, not a bare assignment] [cross: AT-026.11]
...
- **AT-028.14 (P0)** — Given the clarifying-question helper, When invoked on a PM REQUIREMENT, Then a PM-anchored clarifying blocker is raised with the REQ-024 fields; When invoked from a dev item, Then it anchors to the RELATED PM requirement and no dev-item title, status, or comment reaches any NGO-facing surface. [d82v: dev-leak boundary] [cross: AT-024.17]
- **AT-028.15 (P0)** — Given the completion-readiness helper, When invoked, Then it checks: all top-priority PM REQUIREMENTS Done through verified completion (dev-tree state may inform the report but is never the authority), README + runbook present, repo and deployment URLs set, and work pushed — reporting each check's state.
...
- **AT-028.17 (P0)** — Given an ordinary, uncustomized task workflow (no manual naming, no separate generation command), When the volunteer works a pulled task, Then branch and commit conventions are applied automatically, carrying the DEV-ITEM identifier + linking keywords so the GitHub integration links and closes dev leaves on merge (vendor-native, d82); PM-item status is never moved by conventions — the pull and the complete flow are its only authorities; and paired probes: vendor-native dev assignment/status/merge changes STAND untouched, while the same GitHub or manual events aimed at a PM item confer NO PM authority (ignored or detect-and-reverted) [d82v: exemption + hostile-identifier pair]. [cx: uncustomized flow] [T2: corroboration-only finalized] [cross: AT-026.17/AT-008.12]
...
- **AT-028.19 (P0)** — Given the Skill disabled, When each requirement-owned behavior is exercised through its manual route (parameterized matrix: pull a requirement → the platform's manual pull route (produces the SAME authoritative pull record — a raw Linear status edit is NOT a pull and gets reverted); raise blocker/clarifying question → project page; fuel status → project page gauge; comment reading → project page thread; reference files → project page download; complete a requirement → the platform's verified-completion path; close dev leaves → PR merge; completion-readiness → the project page checklist surfaces; scope summary / project status / in-progress PM requirements → the public project page; dev-tree working items → the volunteer's own Linear view only, never the public page), Then each produces its observable result; attribution degrades to unattributed; and the working norms still arrive via the injected prompt. [cx: enumerated matrix] [cx r2: completion-readiness + bootstrap-context routes added] [cross: AT-009.10]
...
| Completion = explicit dev act at PM level; complete flow verifies already-merged dev work + acceptance evidence, then flips PM Done (three defective variants refused) | 09 [d82v] |
...
| Conventions auto-applied (dev leaves only; PM authorities untouched; exemption + hostile-identifier pairs); override anything | 17 [d82v], 18 |
