...
- **AT-011.08 (P0)** — Given an `in_progress` project with a live repo, When the public showcase renders, Then its live card conveys: task-tree progress, cadence, the language/stack, time since last activity, and the assigned contributor; cadence is proven two-input with discriminating fixtures — a task-movement-only period and a commit-only period each move it. [cx r2: discriminating cadence fixtures — a commit-only implementation could pass] [cross: REQ-010.16 owns the same proof on the page]
- **AT-011.14 (P0)** — Given an `in_progress` project whose repo/setup is still pending, When the showcase renders, Then the live card still appears with the available signals populated (task-tree progress, assigned contributor) and explicit pending/unavailable states for repo-dependent values — "a project under build surfaces a live card" is not conditioned on the repo existing. [cx r2: added — the live-repo precondition hid the repo-pending case]
...
| In-progress showcase: live card signals (two-input cadence proven; repo-pending variant renders with pending states) | 08, 14 [cx r2] |
