...
- **AT-011.06 (P0)** — Given an open project that carries funded fuel (fixture), When its card renders, Then no live stats and no dollar figure appear anywhere on the card — static attributes only.
...
- **AT-011.10 (P0)** — Given projects across lifecycle states, When the showcase renders, Then only `in_progress` projects carry the live card: open projects carry the static card, and `completed`/`cancelled` projects appear in neither card set. [cross: REQ-005.5]
...
| Open card: exact static field set; no live stats; no dollar figure; no interest count | 05–07 |
| In-progress showcase: live card signals | 08 |
| Popularity metrics never shown | 09 |
| Card type by lifecycle state (open=static, in_progress=showcase, terminal=neither) | 10 |
