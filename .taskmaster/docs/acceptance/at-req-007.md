...
Source: requirements/req-007.md (prd-mvp.md REQ-007 + Promise §9 disclaimer re-acceptance). Dependencies: REQ-001, REQ-005, REQ-008.
...
- **AT-007.15 (P0)** — Given a consented but unfunded match, When provisioning is probed, Then no kickoff side effect has fired, no usable virtual key has been issued to the volunteer, and no volunteer access (repo/Linear/Tier-2) has been granted — kickoff fires only on funding. Resource-record existence timing is owned by the provisioning requirement (a rematch legitimately inherits a surviving workspace). NGO-side Lovable preparation is not prohibited. [cx: resources named] [cx r2: re-scoped to kickoff/key/access — bare nonexistence failed valid rematch cases] [cross: REQ-005.5/009/027]
- **AT-007.16 (P0)** — Given an `invited` match, When the volunteer declines, Then the match log records the `declined` event with timestamp and reason. [cx r2: narrowed to the stated log obligation — freed/state outcomes are owned by REQ-005.5's table]

## D. Match log

- **AT-007.17 (P0)** — Given a project passing through candidacy, invitation, consent — and separately decline, expiry, and release — When an authenticated platform admin reads the match log through the supported UI/API, Then access succeeds and every one of these event types appears with its timestamp and reason. [cx r2: reader identified — a fixture/DB read proved nothing about admin access]
...
- **AT-007.22 (P0)** — Given an AUP enforcement, When the admin deactivates the account, Then platform writes are rejected immediately and the project's virtual keys are revoked immediately; residual repo, Linear, and org access are removed within 15 minutes (PROVISIONAL AT-defined bound — REQ-007 says only "promptly"; escalated for a founder-pinned number, see open items) with an audit record for each removal; and the NGO receives the prompt to remove the volunteer's Lovable workspace access. [cx: "promptly" pinned to a judgeable bound] [cross: REQ-001/009]
- **AT-007.23 (P0)** — Given a volunteer deactivated through AUP ENFORCEMENT specifically, When the admin re-enables the account and re-issues keys (the manual reversal), Then otherwise-authorized platform writes resume and the newly issued virtual key authenticates — restoration of removed repo/Linear/org/Lovable memberships is NOT asserted (the requirement defines reversal as re-enable + key re-issue only). [cx: narrowed] [cx r2: precondition pinned to the AUP path — the reversal clause belongs to (b), not to voluntary/inactivity deactivation] [cross: REQ-001.31/REQ-009]
- **AT-007.24 (P0)** — Given a volunteer with an active project, inactive just short of 24 months (controlled clock), When the inactivity check runs, Then org membership remains; at the 24-month threshold, Then it is removed via the same path as voluntary deactivation — no immediate account-write or key revocation fires, AND their per-project access on the active project still succeeds when probed, exactly as in the voluntary case. [cx: boundary + path behavior] [cx r2: the "as (a)" persisted-access probe added]
...
- **AT-007.26 (P0)** — Given projects in EVERY lifecycle state other than `open` (`draft`, `discovery_in_progress`, `scoped`, `triage`, `matched_pending_fuel`, `in_progress`, `completed`, `cancelled`), When the OPEN-projects listing renders, Then none of them appears in it — that listing is `open` projects only. The public in-progress showcase is a different surface, owned by REQ-011. [cx: showcase split out] [cx r2: sweep completed — discovery_in_progress and completed were missing] [cross: REQ-005.5/011]
...
| Kickoff only on funding (no kickoff/usable key/volunteer access before); consented match kickoff-ready | 14, 15 |
| Match log admin-only (admin read proven); every event with timestamp + reason; not a public queue (incl. logged-out probe) | 17, 18 |
| Decline/expiry/release recorded | 16, 17 [freed/state outcomes: cross → AT-REQ-005.5.23/40] |
