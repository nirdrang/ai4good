...
- **AT-006.31 (P0)** — Given a funded `in_progress` project, When the NGO cancels, Then the project's provider workspace is archived and the unconsumed balance moves to the general balance in full — released only AFTER the provider's final cost settlement for the workspace lands (a sentinel late-billing entry arriving before settlement reduces the release; nothing is over-credited). [d69: settlement-wait added] [cross: REQ-009]
...
- **AT-006.33 (P0)** — Given a project completing with a known leftover amount, When COMPLETION fires (the event under test, not an assumed release), Then the release happens only after the provider's final cost settlement, and exactly the settled leftover is debited from the project, credited to the general balance, and represented by balanced ledger rows under control totals — a missing, early, or wrong-amount release fails. [cx r2: completion made the trigger; exact-amount both-sides assertion] [d69: settlement-wait added]
...
- **AT-006.48 (P0)** — Given controlled spend fixtures crossing 20%, 5%, and the stop line (provider-truth reads), When each threshold crosses, Then the gauge and threshold events update within the configured propagation bound (provisional ≤5 minutes — founder to pin the "tight cadence" number), the 20% warning and the stop blocker each expose a project-page top-up action, and with no NGO action no card is charged and no stop is lifted — only an NGO-confirmed checkout moves money or re-arms the key. v1 top-up is manual. [cx r2] [d69: spend-limit wording replaced by the key-status stop] [cross: AT-009.13/14; REQ-010 owns the gauge UI]
- **AT-006.52 (P0)** — Given the provider reserve pool is empty or no workspace slot is free (two fixtures), When an NGO attempts to open a funding checkout, Then checkout is blocked BEFORE any payment with the capacity reason, and an admin task exists for replenishment; with a reserved pair available, the same checkout opens and binds the reservation. [d69: added — checkout inventory gate] [cross: REQ-009 owns the pool]
...
| Zero-fuel stop executes at the provider (key inactive, d69) + gateway proxies the rejection | [cross → AT-009.13]; telemetry never money ledger: 30 |
| Checkout requires reserved provider inventory (blocked before payment) | 52 [d69] |
| Release only after final provider cost settlement | 31, 33 [d69] |
