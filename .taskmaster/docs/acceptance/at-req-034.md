...
- **AT-034.01 (P0)** — Given a metered request carrying a valid binding to a pulled PM-tree requirement and one carrying none, When both flow, Then BOTH are served; the bound request's row attributes to its pulled PM requirement, the unbound row records "unattributed" — no resolvable binding is never a rejection reason.
- **AT-034.02 (P0)** — Given an UNRESOLVABLE binding (a requirement item from another project, and a nonexistent item — two fixtures), When each request flows, Then each is served and its row is recorded EXACTLY as "unattributed" with NO requirement association stored; and Given a RESOLVABLE but dishonest binding (a real, CURRENTLY PULLED PM requirement of this project the work does not belong to — third fixture), Then the request is served and the row records against THAT pulled requirement — the telemetry is spoofable by design and never a gate; no truth-checking layer is asserted; and Given a same-project DEV-TREE item (incl. one related to a pulled requirement) and a same-project PM requirement WITHOUT an active pull (two more fixtures), Then each request is served, records exactly unattributed with no dev or PM association and no implicit dev-to-PM roll-up, and raises the binding-failure signal — the two-tree boundary of resolvability. [cx: unattributed pinned] [cx r2: unresolvable vs spoofed separated] [d82v: spoof target = pulled PM requirement; dev-item and unpulled-PM fixtures added]
- **AT-034.03 (P0)** — Given the taskless values, When a session starts without a pulled requirement, Then "exploration" and "onboarding" are offered PROACTIVELY as first-class choices [cross: AT-028.07], and rows from such sessions carry the chosen bucket.
...
- **AT-034.17 (P0)** — Given the binding lifecycle, When probed across four moments: an UNAUTHORIZED manual PM In-Progress edit (no pull) does NOT make that item a resolvable binding target; a dev-tree merge-to-Done leaves the worktree binding on the pulled PM requirement unchanged; after VERIFIED PM completion, reuse of the stale binding records as unattributed (the pull interval ended); and throughout, every request is served — pull AUTHORITY defines validity, never apparent status; attribution never gates. [d82v: added — the pull-interval oracle]
...
- **AT-034.16 (P0)** — Given an exploration-bucket session in which implementation intent appears (edits toward a known PM requirement's scope — fixture), When the conversational steering acts, Then a NON-BLOCKING nudge to select the matching requirement is shown, and When the nudge is NOT followed, Then forwarding remains fully available — nudged, never enforced. [cx r2: added — the nudge clause itself had no test; the Skill-local pause layer is AT-028.08's] [cross: AT-028.08]

## F. Aggregation boundary

- **AT-034.13 (P0)** — Given the NGO burn view, When it renders, Then burn appears per REQUIREMENT and per NGO-facing category, token-denominated, with exploration, onboarding, and unattributed as their OWN visible lines summing to the total (usage never hidden or folded into a requirement); no finer-than-requirement ATTRIBUTION exists — the log holds no dev-item attribution field or association and no NGO surface shows a finer-than-requirement burn line (required surface/actor row metadata remains permitted — the ban is on attribution targets and displayed aggregation, not operational metadata); and Given a POPULATED dev tree (uniquely identifiable seeded dev items with vendor-native status/merge events), Then those items create no attribution target, no baseline dimension, and no NGO burn line — only their pulled PM requirement receives burn; and the rendered view carries NO celebratory copy, badge, animation, or equivalent success treatment — data only. [cx: the no-celebration clause had no assertion]
...
| Pull-interval validity: only the active pull binds; unauthorized PM status ≠ authority; dev merge leaves binding intact until verified completion; stale completed binding → unattributed | 17 [d82v] |
