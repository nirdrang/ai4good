...
- **AT-008.03 (P0)** — Given the project's repo after setup validation with NO active REQ-031 hide (precondition), When it is inspected, Then it is public and carries the MIT license. [d79: public-state tests are scoped to no-active-hide — the hide is the one authorized visibility exception; licensing is never suspended]
...
- **AT-008.13 (P0)** — Given the two trees (d82), When GitHub events flow: a DEV leaf's PR merge flips that leaf Done vendor-natively (and is never reverted), while NO GitHub event moves a PM item — PM In Progress comes only from the pull, PM Done only from verified completion, and an unauthorized manual PM status edit may exist transiently but never becomes authoritative (detected + reverted, final mirror state unchanged). [d82: two-tier status provenance] [cross: REQ-026 owns the Linear rules]

## D. Issues — dev-internal only

- **AT-008.14 (P0)** — Given GitHub Issues exist on the project repo, When EVERY NGO-observable platform channel renders — project page, dashboard, thread, notifications, activity feed, and project-assistant output — Then no GitHub Issue content appears in any of them — the Linear task tree is the NGO-visible source of truth. The public repo itself remains publicly readable (absent an active REQ-031 hide); "never NGO-visible" scopes platform surfaces, not GitHub. [cx: scoped] [cx r2: parameterized across all NGO-observable channels incl. assistant + notifications] [d79: no-active-hide precondition]
...
- **AT-008.20 (P0)** — Given org member A (a volunteer) and another project's repo B, When A attempts a write to B, Then it is rejected; and while B is private — BOTH pre-validation AND under an active REQ-031 hide (two fixtures) — A cannot read it, while B's own assigned volunteer and NGO retain their explicit permissions throughout — org membership grants no repo access by default, and the hide relaxes nothing. [d79: the hidden-repo case added]
- **AT-008.21 (P0)** — Given a member-created repo before setup validation, When its visibility is read, Then it is private; after the platform validates the repo URL (no active hide), Then it is public — desired visibility is EXACT per state: private pre-validation, public post-validation without an active REQ-031 hide, private while a hide is active. [d79: the three-branch desired-state function]
...
- **AT-008.27 (P0)** — Given the exact desired-state function (private pre-validation / public post-validation with no active hide / private while an audited hide is active), When the scheduled check's trigger fires against each drift case (controlled clock — the checker is NOT invoked directly): a validated-public repo flipped private WITHOUT an audit record → remediated back to public; a pre-validation repo flipped public → back to private; an ACTIVE-HIDE repo leaked public → back to PRIVATE (remediation enforces the hide, not just tolerates it); and an authorized hide is distinguished from an arbitrary private flip by the ACTIVE AUDIT RECORD, never inferred from private visibility alone. [cx] [cx r2: scheduled trigger] [d79: bidirectional remediation + audit-record authority — "preserves the hide" permitted skipping hidden repos] [cross: REQ-031]

## H. Sync, export & completion offboarding [cx: added round 1]

- **AT-008.25 (P0)** — Given the linked project repo, When an edit lands on the Lovable side and a commit lands on the GitHub side, Then each is reflected on the other (two-way sync observable in both directions); an anonymous user can clone the public repo, and an authenticated unrelated GitHub user can fork it (both absent an active REQ-031 hide) — MIT + two-way sync keep the code forkable and exportable. [cx r2: anonymous fork impossible — clone anonymous, fork authenticated] [d79: no-active-hide precondition] [cross: REQ-021 owns the Lovable link setup]
...
| Base-permission invariant: exact three-branch desired state (pre-validation private / public / hide private, d66 + d79); non-owner change rejected (all roles); continuously remediated BOTH directions with audit-record authority; hidden repo grants no member access | 20, 21, 22, 27 [d79] |
