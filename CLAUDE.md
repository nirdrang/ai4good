...
- **Work brackets ONE PM-tree requirement item** (Linear team `AI4GOOD-PM`): `/pm-next` pulls it (assign + In Progress + pull-record comment + attribution binding + dev-tree materialization from `loop/decomp/req-0NN.md`); `/pm-done` closes it (evidence gate: the requirement's full AT suite green at integration tier + founder attestation). These verbs are the ONLY status authorities on the PM tree. Dev-board items — PM-requirement leaves and foundation work alike — are built through **`/item-loop`**, the one build lifecycle (brief → adversarial gates → verified merge tail). `/dev-start` and `/dev-end` are that loop's DOORS (open an item / run its merge tail) — ergonomic packaging, never authorities, never an alternative path, and they never touch the PM item (d88, reshaped by AI4DEV-32 founder ruling 2026-07-31: the old leaf verbs' "verification assumed" close is dead).
