...
prose-only per `code-gate-scope.md` — before spawning the fix sitting.

Spawn the FIX-AND-GOAL sitting; its type is in `sittings.md`. Its spawn prompt names every
...
`PHASE-STATE.md` names the audit panel. Then read `audit-tail.md`.
