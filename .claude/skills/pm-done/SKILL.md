---
name: pm-done
description: Complete the currently bound PM-tree requirement — the ONLY authority for Done on the PM board. Reconcile parents + evidence gate (dev items closed + full AT suite green at integration tier) + founder attestation, then the completion record, then Done LAST. (Requirement tier; the leaf-tier close is /dev-end.)
---

# /pm-done — complete the bound requirement (PM board)

**Verb tiers:** PM tree (requirement) = `/pm-next`, `/pm-done`. Dev tree (leaf) = `/dev-start`,
`/dev-end`.

## Ritual (execute in order; ANY gate failure → report the named failure and STOP — /pm-done never partially applies)
...
2. **Binding check.** `Read-Binding` must name the target item (bucket `task`). No binding or
   a different item → STOP (bind first; /pm-done acts only on the pulled requirement). This is
   the worktree-scoping guarantee: `/pm-done` reads only THIS worktree's binding, so it can only
...
4. **Evidence gate — in this order:**
   ...
   c. **Acceptance suite green:** run the manifest's verify command at integration tier and
      ...
      /pm-done cannot pass, by design; say so plainly.
   ...
   *(The same reconcile runs inside `/dev-end` after a leaf merges, so the dev board reads true
   mid-build. `/pm-done` runs it as the guaranteed final sweep regardless.)*
...
9. **Suggest, once (never auto-execute):** this worktree is now free (its binding is cleared),
   ...
   `git worktree remove <this path>`; and (b) offer to pull the next requirement here
   (`/pm-next`). Propose both; do neither without the founder's word.
