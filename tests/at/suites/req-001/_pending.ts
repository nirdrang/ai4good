/**
 * THE IDS REQ-001 HAS NOT LANDED YET, and the leaf that will land each one. There is 1 of them
 * once the append-only-audit leaf is in this tree. There were 3 before that, 6 before that, 11
 * before that, 14 before that, 16 before that, 24 before the three earlier leaves, 28 before that,
 * 30 before that and 33 before that, and the count moves down as leaves land.
 *
 ...
 * sites; there is no partial suite. Thirty-six are written — AT-001.01 through .07 across the first
 ...
 * acknowledgment-identity one, AT-001.21 and .22 with the cross-organisation-denial one,
 * AT-001.23, .40 and .24 with the assigned-volunteer-scope one, AT-001.25, .26, .27, .28 and
 * .35 with the contact-transfer one, AT-001.29, .30 and .31 with the lifecycle-gate one, and
 * AT-001.33 and .34 with the append-only-audit one. The other 1 is declared, not faked: it throws,
 ...
 */
export const LEAF = {
  // ELEVEN LABELS ARE GONE FROM THIS MAP rather than kept for symmetry, and the rule is the same
  // for all eleven: a leaf label sitting here with nothing pointing at it is a claim that something
  // is still pending when nothing is. `tests/at/expected/req-001.json` declares the landed ids
  // green in the same change each time.
  ...
  //           only two ids.
  //   D5.L2 — assigned-volunteer scope, platform-admin reach, logged-out visibility — landed
  //           AT-001.23 and .40 green, and AT-001.24 as capability-pending at both tiers.
  //   D6.L1 — contact transfer, lost-access recovery and the escalation contact — landed
  //           AT-001.25, .26, .27, .28 and .35, its only five ids.
  //   D6.L2 — the lifecycle gate every write route registers through — landed AT-001.29, .30
  //           and .31, its only three ids.
  //   D6.L3 — the append-only audit for role changes and transfer, and sign-in rate limiting —
  //           landed AT-001.33 green at both tiers, and AT-001.34 as capability-pending at both
  //           tiers.
