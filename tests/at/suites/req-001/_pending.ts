 ...
 * once the assigned-volunteer-scope leaf is in this tree. There were 14 before that, 16 before
 * that, 24 before the three earlier leaves, 28 before that, 30 before that and 33 before that,
 * and the count moves down as leaves land.
 *
 ...
 * sites; there is no partial suite. Twenty-six are written — AT-001.01 through .07 across the first
 ...
 * acknowledgment-identity one, AT-001.21 and .22 with the cross-organisation-denial one, and
 * AT-001.23, .40 and .24 with the assigned-volunteer-scope one. The other 11 are declared, not
 * faked: each one throws, loudly, stamped with its own id and with the manifest leaf that will
 * make it real.
 *
 ...
 */
export const LEAF = {
  // EIGHT LABELS ARE GONE FROM THIS MAP rather than kept for symmetry, and the rule is the same
  // for all eight: a leaf label sitting here with nothing pointing at it is a claim that something
