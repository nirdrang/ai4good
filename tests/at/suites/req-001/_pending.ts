/**
 * THE IDS REQ-001 HAS NOT LANDED YET, and the leaf that will land each one. There are 3 of them
 * once the lifecycle-gate leaf is in this tree. There were 6 before that, 11 before that, 14 before
 * that, 16 before that, 24 before the three earlier leaves, 28 before that, 30 before that and 33
 * before that, and the count moves down as leaves land.
 *
 ...
 * sites; there is no partial suite. Thirty-four are written — AT-001.01 through .07 across the first
 ...
 * .35 with the contact-transfer one, and AT-001.29, .30 and .31 with the lifecycle-gate one. The
 * other 3 are declared, not faked: each one throws, loudly, stamped with its own id and with the
 ...
 */
export const LEAF = {
  // TEN LABELS ARE GONE FROM THIS MAP rather than kept for symmetry, and the rule is the same
  // for all ten: a leaf label sitting here with nothing pointing at it is a claim that something
